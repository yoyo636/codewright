...
brew install anomalyco/tap/codewright # macOS 與 Linux（推薦，始終保持最新）
brew install codewright              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S codewright            # Arch Linux (Stable)
paru -S codewright-bin               # Arch Linux (Latest from AUR)
mise use -g codewright               # 任何作業系統
nix run nixpkgs#codewright           # 或使用 github:anomalyco/codewright 以取得最新開發分支
```
...
Codewright 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/codewright/releases) 或 [opencode.ai/download](https://opencode.ai/download) 下載。
...
1. `$CODEWRIGHT_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.codewright/bin` - 預設備用路徑
...
Codewright 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
關於如何設定 Codewright 的詳細資訊，請參閱我們的 [**官方文件**](https://opencode.ai/docs)。

### 參與貢獻

如果您有興趣參與 Codewright 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 Codewright 進行開發

如果您正在開發與 Codewright 相關的專案，並在名稱中使用了 "codewright"（例如 "codewright-dashboard" 或 "codewright-mobile"），請在您的 README 中加入聲明，說明該專案並非由 Codewright 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=52ao9352-5623-4fa0-b7dd-3407c392c1af&qr_code=true) | [X.com](https://x.com/codewright)
