...
- **权限** - 细粒度控制可运行的工具与命令。
- **多端运行** - 终端 TUI、桌面应用（macOS/Windows/Linux）、Web，以及用于脚本与 CI 的无头模式 `codewright run`。
...
# no brew formula yet — build from source              # macOS 和 Linux（官方 brew formula，更新频率较低）
# no pacman package yet — build from source            # Arch Linux (Stable)
# no AUR package yet — build from source               # Arch Linux (Latest from AUR)
# build from source: git clone https://github.com/yoyo636/codewright.git               # 任意系统
# no nix package yet — build from source           # 或用 github:yoyo636/codewright 获取最新 dev 分支
```
...
Codewright 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/yoyo636/codewright/releases) 或 [codewright.dev/download](https://github.com/yoyo636/codewright/download) 下载。
...
1. `$CODEWRIGHT_INSTALL_DIR` - 自定义安装目录
2. `$XDG_BIN_DIR` - 符合 XDG 基础目录规范的路径
3. `$HOME/bin` - 如果存在或可创建的用户二进制目录
4. `$HOME/.codewright/bin` - 默认备用路径
...
```bash
codewright models      # 列出可用模型
```

### 配置

Codewright 从以下来源加载配置：

- 全局：`~/.config/codewright/codewright.json`
- `CODEWRIGHT_CONFIG` 环境变量（配置文件路径）
- 项目：项目中的 `codewright.json` 或 `codewright.jsonc`（向上查找至 worktree 根目录）
...
完整 schema 请查看[官方文档](https://github.com/yoyo636/codewright/docs)。

### Agents

Codewright 内置两种 Agent，可用 `Tab` 键快速切换：
...
了解更多 [Agents](https://github.com/yoyo636/codewright/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://github.com/yoyo636/codewright/docs)。
...
```bash
codewright logs            # 输出最后 50 行日志
codewright logs -n 200     # 输出 200 行
codewright logs --path     # 打印日志文件路径
```

日志文件位于 `~/.local/share/codewright/log/codewright.log`。

**常见问题：**

| 现象 | 解决方法 |
| --- | --- |
| `No AI providers are configured` | 运行 `codewright auth login`，或设置 API Key 环境变量（如 `ANTHROPIC_API_KEY`）。 |
| `Model not found: ...` | 运行 `codewright models`，检查配置中 `provider/model` 拼写。 |
| `Config file at ... is not valid JSON(C)` | 报错会指明行与列；修正 `codewright.json` 中的语法。 |
...
| 变量 | 用途 |
| --- | --- |
| `CODEWRIGHT_LOG_LEVEL` | 日志级别：`DEBUG`、`INFO`（默认）、`WARN`、`ERROR`。 |
| `CODEWRIGHT_PRINT_LOGS` | 设为 `1` 时同时把日志输出到 stderr（并让 `codewright run` 打印完整堆栈）。 |
| `CODEWRIGHT_CONFIG` | 要加载的配置文件路径。 |
| `CODEWRIGHT_CONFIG_DIR` | 加载配置的目录。 |
| `CODEWRIGHT_CONFIG_CONTENT` | 内联配置内容（覆盖文件配置）。 |
| `CODEWRIGHT_AUTH_CONTENT` | 内联 auth JSON（如用于 CI）。 |
| `CODEWRIGHT_DISABLE_PROJECT_CONFIG` | 设为 `1` 时忽略项目级配置。 |
| `CODEWRIGHT_PURE` | 设为 `1` 时不加载外部插件。 |
...
如果你在项目名中使用了 “codewright”（如 “codewright-dashboard” 或 “codewright-mobile”），请在 README 里注明该项目不是 Codewright 团队官方开发，且不存在隶属关系。
