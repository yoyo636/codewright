...
const model = "codewright/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/codewright")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with codewright...`)
  ...
  try {
    await $`codewright run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  codewright failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with codewright")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with codewright...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/codewright`.",
