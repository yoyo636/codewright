...
import { createCodewright } from "@codewright-ai/sdk"
...
`)
  ...
  const codewright = await createCodewright({ port: 0 })
  ...
  try {
    ...
    const session = await codewright.client.session.create()
    const result = await codewright.client.session
      .prompt({
