...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "codewright/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".codewright/glossary/zh-cn.md"
  if (locale === "zht") return ".codewright/glossary/zh-tw.md"
  return `.codewright/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("Codewright did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("Codewright returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("Codewright session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel Codewright runs for 'all' (default: 4)
      --model <provider/id>  Codewright model (default: codewright/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running Codewright
...
`)
  ...
  if (failed.length) console.error(`\nCodewright failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.CODEWRIGHT_DISABLE_PROJECT_CONFIG = "1"
  env.CODEWRIGHT_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["codewright", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.CODEWRIGHT_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["codewright", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.CODEWRIGHT_CONFIG
  delete env.CODEWRIGHT_CONFIG_DIR
  delete env.CODEWRIGHT_CONFIG_CONTENT
  delete env.CODEWRIGHT_PERMISSION
  delete env.CODEWRIGHT_AUTO_SHARE
  return env
}
