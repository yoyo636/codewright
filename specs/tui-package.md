...
Move the canonical Codewright terminal application from
`packages/codewright/src/cli/cmd/tui` into a self-contained workspace package while
...
```text
packages/codewright ---\
                      > @codewright-ai/tui -> @codewright-ai/sdk
packages/cli --------/
```
...
generic presentation libraries. It must not depend on `packages/codewright`,
`packages/cli`, or `@codewright-ai/core`.

The SDK is the TUI's Codewright boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/codewright`.
- Do not replace private `packages/codewright` imports with `@codewright-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@codewright-ai/tui`.

## Ownership Boundary

### `@codewright-ai/tui` Owns
...
- Codewright domain data displayed by the TUI
...
```text
packages/codewright/src/cli/cmd/tui
```

Its private dependency on `packages/codewright` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/codewright/src/*`.
...
Status: Completed. The private `@codewright-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@codewright-ai/tui`.
...
- The package has no dependency on `codewright`, `@codewright-ai/cli`, or
  `@codewright-ai/core`.
...
primitives now live in `@codewright-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@codewright-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Codewright globals or process state;
...
Replace process-global Codewright state with resolved TUI inputs.
...
`@codewright-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/codewright` initially.
...
canonical in `@codewright-ai/tui`. Configured references resolve through the new
...
adapters rather than Codewright domain implementations.

Make the SDK the only Codewright domain boundary used by the TUI.
...
- Domain-facing TUI code imports Codewright data and operations only from
  `@codewright-ai/sdk`.
...
`@codewright-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/codewright`.
...
- The package has no imports from `packages/codewright`, `packages/cli`, or
  `@codewright-ai/core`.
...
public `@codewright-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/codewright`.
- Keep the legacy embedded worker and server startup in `packages/codewright`.
- Change those adapters to load config, create transport inputs, and call the
  public `@codewright-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/codewright`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@codewright-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@codewright-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Codewright domain state.
...
cd packages/codewright && bun typecheck
...
rg '@codewright-ai/core|packages/codewright|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/codewright/src packages/codewright/test
```
...
- Build the current-platform `packages/codewright` binary.
