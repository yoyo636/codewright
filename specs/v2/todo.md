...
The codewright server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@codewright-ai/llm` tool loop after replacing its
...
- plugins get global "codewright" instance like in that post i showed
- codewright instance has stuff like `codewright.session.prompt()` or
  `codewright.tool.register({...})`
...
- decide whether to preserve deprecated `@codewright-ai/llm` orchestration exports
