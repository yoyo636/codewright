...
Both `action` and `resource` use codewright's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.codewright` files is intentionally deferred until `.codewright` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether codewright permits an operation through its managed execution paths.
