...
export async function handler(
    ...
    if (
      ...
      throw new RegionError(
        t("zen.api.error.regionNotAllowed", {
          consoleGoUrl: `https://codewright.dev/workspace/${authInfo.workspaceID}/go`,
        }),
      )
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://codewright.dev/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://codewright.dev/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://codewright.dev/workspace/${authInfo.workspaceID}/members`
