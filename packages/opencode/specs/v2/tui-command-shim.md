...
Delete `packages/codewright/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/codewright/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/codewright/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/codewright`
- TUI plugin loader tests in `packages/codewright` if runtime plugin API wiring changed
