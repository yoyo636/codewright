...
Trim `packages/codewright/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/codewright/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codewright`.
- `[x]` `bun typecheck` from `packages/codewright`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/codewright/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/codewright`.
- `[x]` `bun dev generate > /tmp/codewright-openapi.json` from `packages/codewright`.
...
- Edit `packages/codewright/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/codewright-openapi.json` from `packages/codewright`.
...
- Check source schemas in files like `packages/codewright/src/session/schema.ts`, `packages/codewright/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/codewright/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/codewright/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
