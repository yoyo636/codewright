...
- Author package entrypoint is `@codewright-ai/plugin/tui`.
...
```json
{
  "$schema": "https://codewright.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/codewright-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@codewright-ai/plugin/tui`.
- `@codewright-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
- The value is a semver range checked against the running Codewright version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.codewright` is absent, no check is performed (backward compatible).
...
- `codewright plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `codewright plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.codewright` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.codewright`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.codewright`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.codewright` dir or `CODEWRIGHT_CONFIG_DIR`, Codewright installs `@codewright-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@codewright-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Codewright's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Codewright registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@codewright-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"codewright"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `codewright.default`.
...
- Local plugins persist installed themes under the local `.codewright/themes` area near the plugin config source.
...
- `--pure` / `CODEWRIGHT_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.codewright/plugins/tui-smoke.tsx`
- Local vim plugin: `.codewright/plugins/tui-vim.tsx`
- Local smoke config: `.codewright/tui.json`
- Local smoke theme: `.codewright/plugins/smoke-theme.json`
