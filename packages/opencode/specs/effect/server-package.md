...
Practical reference for a future `packages/server` split after the codewright
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/codewright`.
...
Until enough shared service code lives outside `packages/codewright`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/codewright`

It should not import `packages/codewright` services while `packages/codewright`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on codewright-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/codewright` implementation details.
