...
import { LLMEvent, LLMResponse } from "@codewright-ai/llm"
import { RequestExecutor } from "@codewright-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.codewright.dev/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CODEWRIGHT_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["codewright", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODEWRIGHT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CODEWRIGHT_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["codewright", "native", "oauth", "tool-loop"],
  ...
  {
    id: "codewright-proxy",
    name: "Codewright proxy",
    providerID: ProviderV2.ID.codewright,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["codewright", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CODEWRIGHT_RECORD_CONSOLE_TOKEN && process.env.CODEWRIGHT_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.codewright,
        name: "Codewright Zen",
        env: ["CODEWRIGHT_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CODEWRIGHT_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CODEWRIGHT_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CODEWRIGHT_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["codewright", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CODEWRIGHT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CODEWRIGHT_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CODEWRIGHT_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CODEWRIGHT_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the codewright LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "codewright.json"),
      JSON.stringify({ $schema: "https://codewright.dev/config.json", ...scenario.config(model) }),
    ),
  )
