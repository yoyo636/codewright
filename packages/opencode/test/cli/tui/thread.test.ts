...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ codewright }) =>
    Effect.gen(function* () {
      const result = yield* codewright.spawn(["--replay-limit", "10"])

      codewright.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ codewright }) =>
    Effect.gen(function* () {
      const result = yield* codewright.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ codewright }) =>
    Effect.gen(function* () {
      const result = yield* codewright.spawn(["--mini", "--port", "4096"])
