...
`,
  ...
  const pure = process.env.CODEWRIGHT_PURE
  const meta = process.env.CODEWRIGHT_PLUGIN_META_FILE
  process.env.CODEWRIGHT_PURE = "1"
  process.env.CODEWRIGHT_PLUGIN_META_FILE = tmp.extra.meta
  ...
  try {
    ...
    wait.mockRestore()
    if (pure === undefined) {
      delete process.env.CODEWRIGHT_PURE
    } else {
      process.env.CODEWRIGHT_PURE = pure
    }
    if (meta === undefined) {
      delete process.env.CODEWRIGHT_PLUGIN_META_FILE
    } else {
      process.env.CODEWRIGHT_PLUGIN_META_FILE = meta
    }
  }
})
