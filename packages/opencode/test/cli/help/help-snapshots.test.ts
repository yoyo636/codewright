...
// terminal sizes. The default codewright tui command is excluded —
// `codewright --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `codewright --help` prints today;
...
// top-level help on `--help` and exits 1; not a real codewright command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("codewright CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ codewright }) =>
      Effect.gen(function* () {
        const topLevel = yield* codewright.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* codewright.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`codewright ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`codewright ${argv.join(" ")} --help`)
        }
