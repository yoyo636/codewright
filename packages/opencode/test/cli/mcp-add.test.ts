...
describe("codewright mcp add (non-interactive subprocess)", () => {
  cliIt.concurrent(
    "adds a remote server with HTTP headers",
    ({ home, codewright }) =>
      Effect.gen(function* () {
        const result = yield* codewright.spawn([
          "mcp",
          "add",
          "github",
          "--url",
        ...
        const config = yield* Effect.promise(() =>
          Bun.file(path.join(home, ".config", "codewright", "codewright.json")).json(),
        )
