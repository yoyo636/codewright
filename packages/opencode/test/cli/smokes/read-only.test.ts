...
describe("codewright read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // CODEWRIGHT_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["mcp", "list"])
        codewright.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["providers", "list"])
        codewright.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["models"])
        codewright.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["agent", "list"])
        codewright.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh CODEWRIGHT_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["session", "list"])
        codewright.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["stats"])
        codewright.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under CODEWRIGHT_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ codewright }) =>
      Effect.gen(function* () {
        const r = yield* codewright.spawn(["db", "path"])
        codewright.expectExit(r, 0, "db path")
