    ...
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(url.toString(), dir, {
          ...
          body: JSON.stringify({ $schema: "https://codewright.dev/config.json" }),
        })
        ...
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codewright-directory")
        expect(forwarded[0]?.headers).not.toHaveProperty("x-codewright-workspace")
