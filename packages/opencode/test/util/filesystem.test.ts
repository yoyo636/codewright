...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "codewright.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "codewright.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codewright.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "codewright.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["codewright.json", "codewright.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "codewright.json"),
        path.join(project, "codewright.jsonc"),
      ])
    })
  })
