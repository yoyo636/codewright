...
import { isRecord } from "@codewright-ai/tui/util/record"
...
import { TuiKeybind } from "@codewright-ai/tui/config/keybind"
...
export class Service extends Context.Service<Service, Interface>()("@codewright/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old codewright.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.codewright`
  // folders between cwd and home, and CODEWRIGHT_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CODEWRIGHT_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CODEWRIGHT_TUI_CONFIG override, if set.
  if (Flag.CODEWRIGHT_TUI_CONFIG) {
    const configFile = Flag.CODEWRIGHT_TUI_CONFIG
  ...
  // 4. `.codewright` directories (and CODEWRIGHT_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".codewright") || dir === Flag.CODEWRIGHT_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".codewright") && dir !== Flag.CODEWRIGHT_CONFIG_DIR) continue
