...
import { InstallationLocal, InstallationVersion } from "@codewright-ai/core/installation/version"
import { existsSync } from "fs"
import { Account } from "@/account/account"
import { isRecord } from "@/util/record"
import type { ConsoleState } from "@codewright-ai/core/v1/config/console-state"
import { FSUtil } from "@codewright-ai/core/fs-util"
import { InstanceState } from "@/effect/instance-state"
import { Context, Duration, Effect, Exit, Fiber, Layer, Option, Schema } from "effect"
import { FetchHttpClient, HttpClient, HttpClientRequest } from "effect/unstable/http"
import { EffectFlock } from "@codewright-ai/core/util/effect-flock"
import { containsPath, type InstanceContext } from "../project/instance-context"
import { ConfigV1 } from "@codewright-ai/core/v1/config/config"
import { RemoteAuthError } from "@codewright-ai/core/v1/config/error"
import { ConfigPermissionV1 } from "@codewright-ai/core/v1/config/permission"
...
export class Service extends Context.Service<Service, Interface>()("@codewright/Config") {}

export const use = serviceUse(Service)

function globalConfigFile() {
  const candidates = ["codewright.jsonc", "codewright.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadConfig = Effect.fnUntraced(function* (
      ...
      if (!data.$schema) {
        data.$schema = "https://codewright.dev/config.json"
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      if (!Flag.CODEWRIGHT_CONFIG && !Flag.CODEWRIGHT_CONFIG_DIR && !Flag.CODEWRIGHT_CONFIG_CONTENT) {
        const file = globalConfigFile()
        if (!existsSync(file)) {
          yield* fs
            .writeWithDirs(file, JSON.stringify({ $schema: "https://codewright.dev/config.json" }, null, 2))
            .pipe(Effect.catch(() => Effect.void))
        }
      }
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "config.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codewright.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "codewright.jsonc"), env))
        ...
        yield* Effect.promise(() =>
          import(pathToFileURL(legacy).href, { with: { type: "toml" } })
            .then(async (mod) => {
              ...
              result["$schema"] = "https://codewright.dev/config.json"
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        const pluginScopeForSource = Effect.fnUntraced(function* (source: string) {
          ...
          if (source === "CODEWRIGHT_CONFIG_CONTENT") return "local"
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const wellknownURL = `${url}/.well-known/codewright`
            ...
            if (!remoteConfig.$schema) remoteConfig.$schema = "https://codewright.dev/config.json"
        ...
        if (Flag.CODEWRIGHT_CONFIG) {
          yield* merge(Flag.CODEWRIGHT_CONFIG, yield* loadFile(Flag.CODEWRIGHT_CONFIG, authEnv))
          yield* Effect.logDebug("loaded custom config", { path: Flag.CODEWRIGHT_CONFIG })
        }

        if (!Flag.CODEWRIGHT_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("codewright", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        if (Flag.CODEWRIGHT_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from CODEWRIGHT_CONFIG_DIR", { path: Flag.CODEWRIGHT_CONFIG_DIR })
        }

        const deps: Fiber.Fiber<void>[] = []

        for (const dir of directories) {
          if (dir.endsWith(".codewright") || dir === Flag.CODEWRIGHT_CONFIG_DIR) {
            for (const file of ["codewright.json", "codewright.jsonc"]) {
          ...
          // Auto-discovered plugins under `.codewright/plugin(s)` are already local files, so ConfigPlugin.load
        ...
        if (process.env.CODEWRIGHT_CONFIG_CONTENT) {
          const source = "CODEWRIGHT_CONFIG_CONTENT"
          const next = yield* loadConfig(process.env.CODEWRIGHT_CONFIG_CONTENT, {
            dir: ctx.directory,
            source,
          })
          yield* merge(source, next, "local")
          yield* Effect.logDebug("loaded custom config from CODEWRIGHT_CONFIG_CONTENT")
        }
        ...
        if (activeAccount?.active_org_id) {
          ...
          yield* Effect.gen(function* () {
            ...
            if (Option.isSome(tokenOpt)) {
              process.env["CODEWRIGHT_CONSOLE_TOKEN"] = tokenOpt.value
              yield* env.set("CODEWRIGHT_CONSOLE_TOKEN", tokenOpt.value)
            }
        ...
        if (Flag.CODEWRIGHT_PERMISSION) {
          try {
            result.permission = mergeDeep(result.permission ?? {}, JSON.parse(Flag.CODEWRIGHT_PERMISSION))
          } catch (err) {
            yield* Effect.logWarning("CODEWRIGHT_PERMISSION contains invalid JSON, skipping", { err })
          }
        }
        ...
        if (Flag.CODEWRIGHT_DISABLE_AUTOCOMPACT) {
          result.compaction = { ...result.compaction, auto: false }
        }
        if (Flag.CODEWRIGHT_DISABLE_PRUNE) {
