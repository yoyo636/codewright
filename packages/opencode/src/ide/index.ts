...
import { IdeEvent } from "@codewright-ai/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["CODEWRIGHT_CALLER"] === "vscode" || process.env["CODEWRIGHT_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.codewright"], {
    nothrow: true,
  })
