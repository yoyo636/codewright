...
export type PatchDeps = {
  ...
  files: (dir: string, name: "codewright" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".codewright")
}

function patchName(kind: Kind): "codewright" | "tui" {
  if (kind === "server") return "codewright"
  return "tui"
}
