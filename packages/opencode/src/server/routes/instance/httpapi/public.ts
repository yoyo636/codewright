...
import { CodewrightHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for codewright serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(CodewrightHttpApi)
...
export const PublicApi = CodewrightHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "codewright",
    version: "1.0.0",
    description: "codewright api",
