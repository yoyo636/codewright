...
const directory = "C:/Codewright/NewSessionPanelCorner"
...
test("matches the rounded panel corners to the dark new-session background", async ({ page }, testInfo) => {
  ...
  await page.addInitScript(
    ({ directory, draftID, server }) => {
      ...
      localStorage.setItem("codewright-theme-id", "oc-2")
      localStorage.setItem("codewright-color-scheme", "dark")
      ...
      localStorage.setItem(
        "codewright.window.browser.dat:tabs",
