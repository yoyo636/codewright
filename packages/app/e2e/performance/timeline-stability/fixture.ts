...
import { SessionV1 } from "@codewright-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const directory = "C:/Codewright/TimelineStability"
...
export const model = { providerID: "codewright", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("codewright.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
