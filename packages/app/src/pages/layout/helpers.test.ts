...
import { type Session } from "@codewright-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("codewright://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("codewright://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("codewright://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("codewright://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("codewright://open-project")).toBeUndefined()
    expect(parseDeepLink("codewright://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "codewright://open-project?directory=/a",
      "codewright://other?directory=/b",
      "codewright://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("codewright://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("codewright://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("codewright://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("codewright://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "codewright://new-session?directory=/a",
      "codewright://open-project?directory=/b",
      "codewright://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CODEWRIGHT__: {
        deepLinks: ["codewright://open-project?directory=/a"],
      },
    } as unknown as Window & { __CODEWRIGHT__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["codewright://open-project?directory=/a"])
