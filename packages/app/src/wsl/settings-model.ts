...
export function addServerViewModel(input: {
  ...
  const codewrightCheck = selectedDistro ? (state?.codewrightChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.codewrightChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  codewrightCheck: WslCodewrightCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  if (!addServerCodewrightReady(input.codewrightCheck)) {
    const update = !!input.codewrightCheck?.resolvedPath && input.codewrightCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.codewrightChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.codewrightChecks[item.name]) return [`codewright:${item.name}`]
    return []
  })
