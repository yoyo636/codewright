  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://codewright.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://codewright.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://codewright.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://codewright.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://codewright.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://codewright.example.com"),
      ),
    ).toEqual({
