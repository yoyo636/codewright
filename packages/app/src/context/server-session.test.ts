...
import type { retry } from "@codewright-ai/core/util/retry"
import type { CodewrightEvent, SessionApi } from "@codewright-ai/client/promise"
import type { Message, CodewrightClient, Part, Session } from "@codewright-ai/sdk/v2/client"
...
describe("server session", () => {
  test("projects V2 session events into current and legacy message state", () => {
    ...
    const apply = (input: object) => ctx.store.applyV2(input as CodewrightEvent)
  ...
  test("extends a current page to include the user for split assistant turns", async () => {
    ...
    const store = createServerSession({} as CodewrightClient, {} as SessionApi, messageApi)
    store.remember(session("root"))
