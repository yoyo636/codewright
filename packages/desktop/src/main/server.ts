...
const SIDECAR_SERVICE_NAME = "codewright server"
...
export function preferAppEnv(userDataPath: string) {
  ...
  Object.assign(process.env, {
    ...shellEnv,
    CODEWRIGHT_EXPERIMENTAL_ICON_DISCOVERY: "true",
    CODEWRIGHT_EXPERIMENTAL_FILEWATCHER: "true",
...
export async function checkHealth(url: string, password?: string | null): Promise<boolean> {
  ...
  if (password) {
    const auth = Buffer.from(`codewright:${password}`).toString("base64")
