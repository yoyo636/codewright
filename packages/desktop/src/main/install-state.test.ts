...
describe("hasExistingAppState", () => {
  ...
  test("recognizes state written by an earlier Codewright launch", () => {
    expect(hasExistingAppState([file("codewright.settings")])).toBe(true)
    expect(hasExistingAppState([file("codewright.global.dat")])).toBe(true)
    expect(hasExistingAppState([file("window-state-abc.json")])).toBe(true)
    expect(hasExistingAppState([directory("codewright")])).toBe(true)
  })
})
