...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.codewright.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `codewright.settings.dat` is special: it maps to the `codewright.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"codewright.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // codewright.settings.dat → the electron settings store ("codewright.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "codewright.global.dat").
  const storeName = filename === "codewright.settings.dat" ? "codewright.settings" : filename
