...
import { expectCodewrightVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
import { clearWslDistroState, wslServerIdToRestart } from "./policy"
import {
  installWslDistro,
  installWslCodewright,
...
type WslServersControllerOptions = {
  ...
  resolveCodewright?: typeof resolveWslCodewright
...
export function createWslServersController(
  ...
  const checkCodewright = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolveCodewright ?? resolveWslCodewright)(distro, opts)
  ...
  const refreshCodewrightCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setCodewrightCheck(distro, await checkCodewright(distro, opts))
  }
  ...
  const refreshCodewrightCheckBackground = (id: string, distro: string) => {
    void checkCodewright(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setCodewrightCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl codewright check failed", { id, distro, message })
      })
  }

  const refreshCodewrightChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkCodewright(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setCodewrightCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl codewright check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshCodewrightCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installCodewright(name: string) {
      await runJob({ kind: "install-codewright", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslCodewright(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
...
function codewrightCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "codewright is not installed in this distro",
    }
  }
  if (!version) {
    return {
