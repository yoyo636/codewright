...
let releaseCodewrightResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectCodewrightVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectCodewrightVersion("1.14.35", "1.16.2")).toThrow(
    "Codewright update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating Codewright", () => {
  expect(
    wslServerIdToRestart(
      [
        {
  ...
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.codewright/bin/codewright",
          version: "1.16.2",
...
test("ignores stale background Codewright checks after removing a WSL server", async () => {
  persistedServers = []
  releaseCodewrightResolve = undefined
  ...
  await waitFor(() => !!releaseCodewrightResolve)
  await controller.removeServer("wsl:Debian")
  releaseCodewrightResolve?.()
...
test("probes addable distros in parallel before checking Codewright", async () => {
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveCodewright: async (distro) => {
      codewright.push(distro)
      return "/home/me/.codewright/bin/codewright"
    },
  })
...
function testControllerOptions() {
  return {
    ...
    resolveCodewright: async () => {
      await new Promise<void>((resolve) => {
        releaseCodewrightResolve = resolve
      })
      return "/home/me/.codewright/bin/codewright"
    },
  }
