...
function ref(request: HttpServerRequest.HttpServerRequest): Location.Ref {
  ...
  const workspaceID = query.get("location[workspace]") || request.headers["x-codewright-workspace"]
  const directory =
    query.get("location[directory]") ||
    (request.headers["x-codewright-directory"] ? decode(request.headers["x-codewright-directory"]) : process.cwd())
