---
title: Plugins
description: Skriv dine egne plugins for at udvide Codewright.
---

Plugins giver dig mulighed for at udvide Codewright ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre Codewrights standardadfærd.
...
- `.codewright/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/codewright/node_modules/`.
...
1. Global konfiguration (`~/.config/codewright/codewright.json`)
2. Projektkonfiguration (`codewright.json`)
3. Global plugin-mappe (`~/.config/codewright/plugins/`)
4. Projekt plugin bibliotek (`.codewright/plugins/`)
...
Codewright kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En codewright SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide codewright.
...
:::note
Hvis du bruger Codewright desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå codewright i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til codewright:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som codewright kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for codewright sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://codewright.dev/docs/sdk) for detaljer.
