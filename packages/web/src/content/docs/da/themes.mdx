...
Med Codewright kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger Codewright vårt eget `codewright`-tema.
...
Codewright kommer med flere innebygde temaer.
...
- Vil at Codewright skal matche terminalens utseende
...
Codewright støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/codewright/themes/*.json` eller `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.codewright/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.codewright/themes/*.json`
