---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

Codewright bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i Codewright-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/codewright/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i Codewright-konfigurationen.
...
Codewright Zen er en liste over modeller leveret af Codewright-teamet, som er blevet testet og verificeret til at fungere godt med Codewright. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Codewright Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg codewright og gå til [codewright.dev/auth](https://codewright.dev/auth).
...
Det fungerer som alle andre udbydere i Codewright og er helt valgfrit at bruge.

---

## Codewright Go

Codewright Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af Codewright-teamet, som er testet og verificeret til at fungere godt med Codewright.

1. Kør kommandoen `/connect` i TUI, vælg `Codewright Go`, og gå til [codewright.dev/auth](https://codewright.dev/zen).
...
Sådan bruger du Amazon Bedrock med Codewright:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører codewright:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `codewright.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Codewright understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codewright til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at codewright skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem codewright-konfigurationen.
...
gemme tokenet i codewright auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Codewright bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Codewright
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`codewright.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `codewright.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Codewright:
...
Sådan bruger du Google Vertex AI med Codewright:
...
2. Angiv de nødvendige miljøvariabler:
   ...
   Sæt dem mens du kører codewright.
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem codewright, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [codewright-helicone-session](https://github.com/H2Shami/codewright-helicone-session) plugin for automatisk at logge hver Codewright-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver Codewright-samtale opført som en separat session.
...
Du kan konfigurere codewright til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere codewright til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere codewright til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Codewright. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/codewright) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Codewright:
...
3. Kopier API-nøglen til brug i Codewright.
...
6. **Vigtigt**: Før du bruger skymodeller i Codewright, skal du hente modelinformationen lokalt:
...
Codewright Zen er en liste over testede og verificerede modeller leveret af Codewright-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Codewright Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Codewright Zen**.

   ```txt
   /connect
   ```

3. Indtast Codewright API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem codewright-konfigurationen.
...
5. Du kan også tilpasse dem gennem codewright-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Codewright:
...
Du kan også tilpasse modeller gennem codewright-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med codewright. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `codewright.json`-filen i projektkataloget:
...
`limit`-felterne lader Codewright forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `codewright auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek codewright-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i codewright-konfigurationen.
