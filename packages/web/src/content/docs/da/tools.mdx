...
Her er alle de innebygde verktøyene som er tilgængelige i Codewright.
...
:::note
Dette verktøyet er bare tilgængelig når `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (eller `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger Codewright-leverandøren eller når miljøvariabelen `CODEWRIGHT_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
