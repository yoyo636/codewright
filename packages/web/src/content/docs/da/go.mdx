...
Codewright Go er et lavprisabonnement — **$5 for din første måned**, derefter **$10/måned** — der giver dig pålidelig adgang til populære åbne kodningsmodeller.

Go fungerer som enhver anden udbyder i Codewright. Du abonnerer på Codewright Go og
...
:::tip
Vi testede en udvalgt gruppe af modeller og udbydere, der fungerer godt med Codewright.
:::
...
Codewright Go giver dig adgang til disse modeller for **$5 for din første måned**, derefter **$10/måned**.

---

## Sådan fungerer det

Codewright Go fungerer som enhver anden udbyder i Codewright.

1. Du logger ind på **<a href={console}>Codewright Zen</a>**, abonnerer på Go, og
   kopierer din API-nøgle.
2. Du kører kommandoen `/connect` i TUI'en, vælger `Codewright Go`, og indsætter
   din API-nøgle.
3. Kør `/models` i TUI'en for at se listen over tilgængelige modeller gennem Go.

:::note
Kun ét medlem per arbejdsområde kan abonnere på Codewright Go.
:::
...
Codewright Go inkluderer følgende grænser:
...
Dit [model id](/docs/config/#models) i din Codewright config
bruger formatet `codewright-go/<model-id>`. For eksempel for Kimi K3, vil du
bruge `codewright-go/kimi-k3` i din config.
...
Vi skabte Codewright Go for at:
...
4. Undgå **lock-in** ved også at lade dig bruge enhver anden udbyder med Codewright.
