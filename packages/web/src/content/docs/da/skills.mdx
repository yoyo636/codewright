...
Agentferdigheter lar Codewright oppdage gjenbrugbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Codewright søker etter disse stedene:

- Prosjektkonfigurasjon: `.codewright/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/codewright/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Codewright opp fra din nåværende arbejdskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.codewright/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.codewright/skills/git-release/SKILL.md` slik:
...
Codewright viser tilgængelige ferdigheter i `skill` verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved at bruge mønsterbaserte tillatelser i `codewright.json`:
...
**For innebygde agenter** (i `codewright.json`):
