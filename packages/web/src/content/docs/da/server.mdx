---
title: Server
description: Interager med codewright-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codewright serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en codewright-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `codewright.local` |
...
Indstil `CODEWRIGHT_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `codewright`, eller indstil `CODEWRIGHT_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `codewright serve` og `codewright web`.
...
Når du kører `codewright` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug codewright-serveren til at interagere med codewright programmatisk.
:::

Denne arkitektur lader codewright understøtte flere klienter og lader dig interagere med codewright programmatisk.

Du kan køre `codewright serve` for at starte en selvstændig server. Hvis du har
codewright TUI kørende, vil `codewright serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Codewright [IDE](/docs/ide) plugins.
...
Codewright-serveren viser følgende API'er.
