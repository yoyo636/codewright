...
Der er to typer agenter i Codewright; primære agenter og subagenter.
...
Codewright leveres med indbyggede primære agenter, **Build** og **Plan**. Vi vil
...
Codewright leveres med tre indbyggede subagenter, **General**, **Explore** og **Scout**. Vi ser nærmere på dem nedenfor.

---

## Indbyggede

Codewright leveres med to indbyggede primære agenter og tre indbyggede subagenter.
...
En skrivebeskyttet agent til eksterne docs og research af dependencies. Brug denne, når du har brug for at klone et dependency-repository ind i Codewright's administrerede cache, inspicere kildekoden i et bibliotek eller krydstjekke lokal kode mod upstream-implementeringer uden at ændre dit workspace.
...
Konfigurer agent i din `codewright.json`-konfigurationsfil:
...
- Globalt: `~/.config/codewright/agents/`
- Pr. projekt: `.codewright/agents/`
...
Hvis ingen temperatur er angivet, bruger Codewright modelspecifikke standardindstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne sti er i forhold til, hvor konfigurationsfilen er placeret. Så dette virker både for den globale Codewright-konfiguration og den projektspecifikke konfiguration.
...
Model-id'et i din Codewright-konfiguration brugerformatet `provider/model-id`. For eksempel, hvis du bruger [Codewright Zen](/docs/zen), vil du bruge `codewright/gpt-5.1-codex` til GPT 5.1 Codex.
...
:::tip
Kør `codewright models` for at se en liste over de tilgængelige modeller.
:::
...
:::tip
Har du en agent, du gerne vil dele? [Send en PR](https://github.com/anomalyco/codewright).
:::
