---
title: LSP Servers
description: Codewright integrates with your LSP servers.
---

Codewright can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Codewright comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CODEWRIGHT_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and codewright opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your codewright config.
...
```json title="codewright.json" {5-8}
{
  "$schema": "https://codewright.dev/config.json",
  "lsp": {
    "rust": {
...
```json title="codewright.json" {5-13}
{
  "$schema": "https://codewright.dev/config.json",
  "lsp": {
    "custom-lsp": {
