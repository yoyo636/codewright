...
Create `.codewright/commands/test.md`:
...
You can add custom commands through the Codewright config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Codewright [config](/docs/config):
...
- Per-project: `.codewright/commands/`
...
codewright includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
