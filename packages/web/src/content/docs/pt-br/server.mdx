---
title: Servidor
description: Interaja com o servidor codewright via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `codewright serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente codewright pode usar.
...
```bash
codewright serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `codewright.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
codewright serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CODEWRIGHT_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `codewright`, ou defina `CODEWRIGHT_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `codewright serve` quanto ao `codewright web`.

```bash
CODEWRIGHT_SERVER_PASSWORD=your-password codewright serve
```
...
Quando você executa `codewright`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor codewright para interagir com o codewright programaticamente.
:::

Essa arquitetura permite que o codewright suporte múltiplos clientes e permite que você interaja com o codewright programaticamente.

Você pode executar `codewright serve` para iniciar um servidor autônomo. Se você tiver o TUI do codewright em execução, `codewright serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do codewright [IDE](/docs/ide).
...
O servidor codewright expõe as seguintes APIs.
