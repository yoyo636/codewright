...
Com o codewright, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o codewright usa nosso próprio tema `codewright`.
...
O codewright vem com vários temas embutidos.
...
- Querem que o codewright corresponda à aparência do seu terminal
...
O codewright suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/codewright/themes/*.json` ou `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.codewright/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.codewright/themes/*.json`
