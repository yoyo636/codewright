---
title: Plugins
description: Escreva seus próprios plugins para estender o codewright.
---

Plugins permitem que você estenda o codewright conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do codewright.
...
- `.codewright/plugins/` - Plugins em nível de projeto
- `~/.config/codewright/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/codewright/node_modules/`.
...
1. Configuração global (`~/.config/codewright/codewright.json`)
2. Configuração do projeto (`codewright.json`)
3. Diretório de plugins global (`~/.config/codewright/plugins/`)
4. Diretório de plugins do projeto (`.codewright/plugins/`)
...
O codewright executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do codewright para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o codewright.
...
:::note
Se você estiver usando o aplicativo desktop codewright, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o codewright de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao codewright:
...
O helper `tool` cria uma ferramenta personalizada que o codewright pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o codewright junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://codewright.dev/docs/sdk) para detalhes.
