...
Existem dois tipos de agentes no codewright; agentes primários e subagentes.
...
codewright vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Codewright vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Codewright vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Codewright, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `codewright.json`:
...
- Global: `~/.config/codewright/agents/`
- Por projeto: `.codewright/agents/`

```markdown title="~/.config/codewright/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o codewright usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do codewright quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do codewright usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Codewright Zen](/docs/zen), você usaria `codewright/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="codewright.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="codewright.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
```json title="codewright.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `codewright models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/codewright).
:::
...
```markdown title="~/.config/codewright/agents/docs-writer.md"
---
...
```markdown title="~/.config/codewright/agents/security-auditor.md"
---
