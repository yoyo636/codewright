...
O Codewright Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Codewright. Você assina o Codewright Go e
...
O Codewright Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Codewright Go funciona como qualquer outro provedor no Codewright.

1. Você entra no **<a href={console}>Codewright Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Codewright Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Codewright Go.
:::
...
O Codewright Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Codewright
usa o formato `codewright-go/<model-id>`. Por exemplo, para o Kimi K3, você usaria
`codewright-go/kimi-k3` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Codewright também.
