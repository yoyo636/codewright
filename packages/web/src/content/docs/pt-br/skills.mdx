...
As habilidades do agente permitem que o codewright descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O codewright pesquisa nesses locais:

- Configuração do projeto: `.codewright/skills/<name>/SKILL.md`
- Configuração global: `~/.config/codewright/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o codewright sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.codewright/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.codewright/skills/git-release/SKILL.md` assim:
...
O codewright lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `codewright.json`:
...
**Para agentes embutidos** (em `codewright.json`):
