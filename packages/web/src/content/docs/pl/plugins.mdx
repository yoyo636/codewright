---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć codewright.
---

Wtyczki umożliwiające wyłączenie codewright poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania codewright.
...
- `.codewright/plugins/` - Wtyczki na poziomie projektu
- `~/.config/codewright/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/codewright/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/codewright/codewright.json`)
2. Konfiguracja projektu (`codewright.json`)
3. Globalny katalog wtyczek (`~/.config/codewright/plugins/`)
4. Katalog wtyczek projektu (`.codewright/plugins/`)
...
codewright uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z codewright do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do codewright.
...
:::note
Jeśli korzystasz z aplikacji komputerowej codewright, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj codewright czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do codewright:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod codewright. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://codewright.dev/docs/sdk).
