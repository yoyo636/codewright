...
Możesz dodać dodatkowe informacje do codewright za pomocą _Model Context Protocol_, w skrócie MCP. codewright obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [codewright Config](https://codewright.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/codewright`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
codewright automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, codewright:
...
Jeśli serwer wymaga uwierzytelnienia, codewright poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `codewright mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji codewright bezpieczne przechowa tokeny w `~/.local/share/codewright/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w codewright, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie codewright, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć codewright z kontem Sentry.
