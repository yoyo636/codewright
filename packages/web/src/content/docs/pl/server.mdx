---
title: Serwer
description: Interakcja z serwerem codewright za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `codewright serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient codewright.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `codewright.local` |
...
Ustaw `CODEWRIGHT_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `codewright` lub ustaw `CODEWRIGHT_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `codewright serve`, jak i `codewright web`.
...
Kiedy uruchomisz `codewright`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera codewright do programowej interakcji z codewright.
:::

Ta architektura umożliwia obsługę wielu klientów przez codewright i programową interakcję z kodem otwartym.

Możesz uruchomić `codewright serve`, aby uruchomić samodzielny serwer. Jeśli masz
codewright TUI działa, `codewright serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki codewright [IDE](/docs/ide).
...
Serwer codewright udostępnia następujące interfejsy API.
