...
Umiejętności agenta pozwalają codewright odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
codewright przeszukuje te lokalizacje:

- Project config: `.codewright/skills/<name>/SKILL.md`
- Global config: `~/.config/codewright/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu codewright przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.codewright/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.codewright/skills/git-release/SKILL.md` w ten sposób:
...
codewright wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `codewright.json`:
...
**Dla agentów wbudowanych** (w `codewright.json`):
