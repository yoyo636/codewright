...
Dzięki codewright możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie codewright używa naszego własnego motywu `codewright`.
...
codewright ma kilka wbudowanych motywów.
...
- Chcą, aby Codewright pasował do wyglądu ich terminala
...
codewright obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/codewright/themes/*.json` lub `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.codewright/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.codewright/themes/*.json`
