...
Codewright Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w Codewright. Subskrybujesz Codewright Go i
...
Codewright Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

Codewright Go działa jak każdy inny dostawca w Codewright.

1. Logujesz się do **<a href={console}>Codewright Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `Codewright Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować Codewright Go.
:::
...
Codewright Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji Codewright
używa formatu `codewright-go/<model-id>`. Na przykład dla Kimi K3 należy użyć
`codewright-go/kimi-k3` w swojej konfiguracji.
...
Stworzyliśmy Codewright Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z Codewright.
