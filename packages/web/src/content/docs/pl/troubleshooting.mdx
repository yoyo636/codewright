...
Aby debugować problemy z codewright, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codewright\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `codewright --log-level DEBUG`.

---

## Przechowywanie danych

codewright przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/codewright/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\codewright`
...
codewright Desktop uruchamia w tle lokalny serwer codewright (sidecar `codewright-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Codewright` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/codewright/codewright.jsonc` (lub `~/.config/codewright/codewright.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/codewright/codewright.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codewright\codewright.jsonc`
...
codewright może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/codewright/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\codewright\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby codewright mógł ją odbudować.

1. Całkowicie zamknij codewright Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codewright`
- **Linux**: usuń `~/.cache/codewright` (lub uruchom `rm -rf ~/.cache/codewright`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\codewright`

3. Uruchom ponownie codewright Desktop.

---

### Napraw problemy z połączeniem z serwerem

codewright Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `codewright.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CODEWRIGHT_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CODEWRIGHT_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows codewright Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami codewright.

---

### Brak powiadomień

codewright Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla codewright w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie codewright Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji codewright Desktop):

- `codewright.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `codewright.global.dat` i `codewright.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
Jeśli masz problemy z codewright:
...
3. Upewnij się, że masz najnowszą wersję z `codewright upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `codewright models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\codewright`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. codewright dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\codewright`

2. Uruchom ponownie kod codewright, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to codewright do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
codewright wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
