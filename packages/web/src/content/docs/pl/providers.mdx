---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

codewright używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji codewright.
...
w `~/.local/share/codewright/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim codewright
config.
...
Codewright Zen to lista modeli dostarczonych przez zespół codewright, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z codewright. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Codewright Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Codewright Zen` i przejdź do [codewright.dev/auth](https://codewright.dev/zen).
...
Działa jak każdy inny dostawca w codewright i jest całkowicie opcjonalny w użyciu.

---

## Codewright Go

Codewright Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół codewright, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z codewright.

1. Uruchom polecenie `/connect` w TUI, wybierz `Codewright Go` i przejdź do [codewright.dev/auth](https://codewright.dev/zen).
...
Aby używać Amazon Bedrock z codewright:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania codewright:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `codewright.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w codewright nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować codewright do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod codewright działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji codewright.
...
codewright używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować codewright
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`codewright.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem codewright:
...
Aby używać Google Vertex AI z codewright:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania codewright.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą codewright, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [codewright-helicone-session](https://github.com/H2Shami/codewright-helicone-session), aby automatycznie rejestrować każdą konwersację codewright jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja codewright będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować codewright tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować codewright tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować codewright tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem codewright. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/codewright).
:::
...
Aby korzystać z Ollama Cloud z codewright:
...
3. Skopiuj klucz API do użycia w codewright.
...
6. **Ważne**: Przed użyciem modeli chmurowych w codewright musisz pobrać informacje o modelu lokalnie:
...
Codewright Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół codewright. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Codewright Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Codewright Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API codewright.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji codewright.
...
5. Możesz je także dostosować za pomocą konfiguracji codewright. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z codewright:
...
Możesz także dostosować modele za pomocą konfiguracji codewright. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z codewright. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `codewright.json` w katalogu projektu:
...
Pola `limit` pozwalają codewright zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `codewright auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację codewright i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji codewright.
