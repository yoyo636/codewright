...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Codewright posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Codewright.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (lub `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Codewright lub gdy zmienna środowiskowa `CODEWRIGHT_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Codewright:
