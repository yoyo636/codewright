...
Создайте `.codewright/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию codewright или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем codewright [config](/docs/config):
...
- Глобальный: `~/.config/codewright/commands/`
- Для каждого проекта: `.codewright/commands/`
...
codewright включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
