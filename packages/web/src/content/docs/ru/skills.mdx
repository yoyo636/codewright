...
Навыки агента позволяют codewright обнаруживать многократно используемые инструкции из вашего репозитория или домашнего каталога.
...
codewright выполняет поиск в следующих местах:

- Конфигурация проекта: `.codewright/skills/<name>/SKILL.md`
- Глобальная конфигурация: `~/.config/codewright/skills/<name>/SKILL.md`.
...
Для локальных путей проекта codewright переходит из вашего текущего рабочего каталога, пока не достигнет рабочего дерева git.
Он загружает все соответствующие `skills/*/SKILL.md` в `.codewright/` и все соответствующие `.claude/skills/*/SKILL.md` или `.agents/skills/*/SKILL.md` по пути.

Глобальные определения также загружаются из `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` и `~/.agents/skills/*/SKILL.md`.
...
Создайте `.codewright/skills/git-release/SKILL.md` следующим образом:
...
codewright перечисляет доступные навыки в описании инструмента `skill`.
...
Контролируйте, к каким навыкам агенты могут получить доступ, используя разрешения на основе шаблонов в `codewright.json`:
...
**Для встроенных агентов** (в формате `codewright.json`):
