---
title: Policies
description: Control which configured resources Codewright may use.
---

Policies control whether Codewright may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `codewright.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether Codewright may use a resource such as an LLM provider.
...
Codewright currently supports one policy action:
