---
title: TUI
description: Using the Codewright terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codewright provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Codewright starts the TUI for the current directory.
...
When using the Codewright TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Codewright. Allows you to select from available providers and add their API keys.
...
Exit Codewright. _Aliases_: `/quit`, `/q`
...
This is separate from `codewright.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Codewright waits after the leader key. Defaults to `2000`.
...
Use `CODEWRIGHT_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `codewright.default`.
