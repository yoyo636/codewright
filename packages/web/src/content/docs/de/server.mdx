---
title: Server
description: Nutze den codewright-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `codewright serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein codewright-Client nutzen kann.
...
Setze `CODEWRIGHT_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `codewright`, kann aber mit `CODEWRIGHT_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `codewright serve` und `codewright web`.
...
Wenn du `codewright` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den codewright-Server, um codewright programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `codewright serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `codewright serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die Codewright-[IDE](/docs/ide)-Plugins.
...
Der codewright-Server stellt folgende APIs bereit.
