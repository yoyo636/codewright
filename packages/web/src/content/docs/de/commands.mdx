...
Erstellen Sie `.codewright/commands/test.md`:

```md title=".codewright/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die Codewright-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem Codewright [config](/docs/config):

```json title="codewright.jsonc" {4-12}
{
...
- Global: `~/.config/codewright/commands/`
- Pro Projekt: `.codewright/commands/`

```markdown title="~/.config/codewright/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".codewright/commands/component.md"
---
...
```md title=".codewright/commands/create-file.md"
---
...
```md title=".codewright/commands/analyze-coverage.md"
---
...
```md title=".codewright/commands/review-changes.md"
---
...
```md title=".codewright/commands/review-component.md"
---
...
Codewright enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
