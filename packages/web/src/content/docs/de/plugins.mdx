---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um Codewright zu erweitern.
---

Mit Plugins können Sie Codewright erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von Codewright zu ändern.
...
- `.codewright/plugins/` – Plugins auf Projektebene
- `~/.config/codewright/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/codewright/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/codewright/codewright.json`)
2. Projektkonfiguration (`codewright.json`)
3. Globales Plugin-Verzeichnis (`~/.config/codewright/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.codewright/plugins/`)
...
Codewright führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein Codewright-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie Codewright erweitern können.
...
:::note
Wenn Sie die Codewright-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass Codewright `.env`-Dateien liest:
...
Plugins können Codewright auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das Codewright aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für Codewright zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://codewright.dev/docs/sdk).
