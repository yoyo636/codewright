...
description: Erstellen Sie Tools, die der LLM in Codewright aufrufen kann.
---

Benutzerdefinierte Tools sind von Ihnen erstellte Funktionen, die der LLM während Gesprächen aufrufen kann. Sie arbeiten neben [built-in tools](/docs/tools) von Codewright wie `read`, `write` und `bash`.
...
- Lokal, indem Sie sie im Verzeichnis `.codewright/tools/` Ihres Projekts platzieren.
- Oder global, indem Sie sie in `~/.config/codewright/tools/` platzieren.
