---
title: Regeln
description: Lege eigene Anweisungen fuer codewright fest.
---

Du kannst codewright ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in codewright den Befehl `/init` aus.
...
So kann codewright sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht codewright sie zu erweitern.
...
codewright kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/codewright/AGENTS.md` speichern.
Diese gelten in allen codewright-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Codewright dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/codewright/AGENTS.md` existiert)
...
Beim Start sucht codewright in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/codewright/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `codewright.json` oder in `~/.config/codewright/codewright.json` angeben.
...
codewright parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit codewright.json

Empfohlen ist das `instructions`-Feld in der `codewright.json`:
...
Du kannst codewright explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `codewright.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
