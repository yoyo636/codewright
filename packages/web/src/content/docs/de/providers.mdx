---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in Codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

Codewright verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer Codewright-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem Codewright anpassen
config.
...
Codewright Zen ist eine Liste von Modellen, die vom Codewright-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit Codewright funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit Codewright Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie Codewright aus und gehen Sie zu [codewright.dev/auth](https://codewright.dev/auth).
...
Es funktioniert wie jeder andere Anbieter in Codewright und ist völlig optional.

---

## Codewright Go

Codewright Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom Codewright-Team getestet und verifiziert wurden, dass sie gut mit Codewright funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `Codewright Go` und gehen Sie zu [codewright.dev/auth](https://codewright.dev/zen).
...
So verwenden Sie Amazon Bedrock mit Codewright:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie Codewright ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `codewright.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in Codewright wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können codewright so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit Codewright ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre Codewright-Konfiguration hinzufügen.
...
2. Wählen Sie Ihre Authentifizierungsmethode:
   ...
   3. Name: `Codewright`, Bereiche: `api`
...
um das Token im Codewright-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
Codewright verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von Codewright
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`codewright.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `codewright.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit Codewright:
...
So verwenden Sie Google Vertex AI mit Codewright:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie Codewright ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über Codewright konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede Codewright-Konversation als separate Sitzung aufgeführt.
...
Sie können Codewright für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können Codewright über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können Codewright über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für Codewright konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/codewright).
:::
...
So verwenden Sie Ollama Cloud mit Codewright:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in Codewright.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in Codewright verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
Codewright Zen ist eine Liste getesteter und verifizierter Modelle, die vom Codewright-Team bereitgestellt werden. [Learn more](/docs/zen).

1. Melden Sie sich bei **<a href={console}>Codewright Zen</a>** an und klicken Sie auf **Create API Key**.

2. Führen Sie den Befehl `/connect` aus und suchen Sie nach **Codewright Zen**.

   ```txt
   /connect
   ```

3. Geben Sie Ihren Codewright API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre Codewright-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre Codewright-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit Codewright:
...
Sie können Modelle auch über Ihre Codewright-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit Codewright verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `codewright.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann Codewright erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `codewright auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die Codewright-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer Codewright-Konfiguration übereinstimmt.
