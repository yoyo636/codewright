...
Wenn Codewright Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codewright\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `codewright --log-level DEBUG`.

---

## Speicher

codewright speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/codewright/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\codewright`
...
Codewright Desktop startet im Hintergrund einen lokalen Codewright-Server (`codewright-cli`-Sidecar).
...
- Nur macOS: `Codewright`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/codewright/codewright.jsonc` (oder `~/.config/codewright/codewright.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/codewright/codewright.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codewright\codewright.jsonc`
...
Codewright kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/codewright/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\codewright\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Codewright ihn neu aufbauen kann.

1. Beende Codewright Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/codewright`
- **Linux**: loesche `~/.cache/codewright` (oder fuehre aus: `rm -rf ~/.cache/codewright`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\codewright`

3. Starte Codewright Desktop neu.

---

### Server-Verbindungsprobleme beheben

Codewright Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `codewright.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CODEWRIGHT_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CODEWRIGHT_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Codewright Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit Codewright-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Codewright Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Codewright in den OS-Einstellungen aktiviert sind, und
...
1. Beende Codewright Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Codewright Desktop):

- `codewright.settings.dat` (desktop default server URL)
- `codewright.global.dat` und `codewright.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Codewright hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `codewright models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\codewright`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. codewright installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\codewright`

2. Starte codewright neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt codewright, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
codewright erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
