...
Mit Codewright kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Codewright das `codewright`-Theme.
...
Codewright liefert mehrere eingebaute Themes mit.
...
- Codewright optisch ans Terminal anpassen wollen
...
Codewright unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/codewright/themes/*.json` oder `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.codewright/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.codewright/themes/*.json`
