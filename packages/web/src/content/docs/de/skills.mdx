...
Agent Skills erlauben Codewright, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Codewright sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Codewright vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.codewright/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.codewright/skills/git-release/SKILL.md` zum Beispiel so:
...
Codewright listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `codewright.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `codewright.json`):
