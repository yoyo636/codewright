---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

Codewright bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i Codewright-konfigurasjonen.
...
i `~/.local/share/codewright/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i Codewright
konfig.
...
Codewright Zen er en liste over modeller levert av Codewright-teamet som har vært
testet og verifisert for å fungere godt med Codewright. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Codewright Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Codewright Zen` og gå til [codewright.dev/auth](https://codewright.dev/zen).
...
Det fungerer som alle andre leverandører i Codewright og er helt valgfritt å bruke.

---

## Codewright Go

Codewright Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av Codewright-teamet som har vært
testet og verifisert for å fungere godt med Codewright.

1. Kjør kommandoen `/connect` i TUI, velg `Codewright Go`, og gå til [codewright.dev/auth](https://codewright.dev/zen).
...
Slik bruker du Amazon Bedrock med Codewright:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Codewright:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `codewright.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i codewright støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere codewright til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Codewright skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom Codewright-konfigurasjonen.
...
for å lagre token i Codewright auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Codewright bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Codewright
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`codewright.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `codewright.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Codewright:
...
Slik bruker du Google Vertex AI med Codewright:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Codewright.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Codewright, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [codewright-helicone-session](https://github.com/H2Shami/codewright-helicone-session) plugin for automatisk å logge hver Codewright-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver Codewright-samtale oppført som en separat økt.
...
Du kan konfigurere Codewright for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Codewright for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Codewright for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Codewright. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/codewright) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Codewright:
...
3. Kopier API-nøkkelen for bruk i Codewright.
...
6. **Viktig**: Før du bruker skymodeller i Codewright, må du hente modellinformasjonen lokalt:
...
Codewright Zen er en liste over testede og verifiserte modeller levert av Codewright-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Codewright Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Codewright Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Codewright API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom Codewright-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom Codewright-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Codewright:
...
Du kan også tilpasse modeller gjennom Codewright-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Codewright. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `codewright.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Codewright forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `codewright auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk Codewright-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i Codewright-konfigurasjonen.
