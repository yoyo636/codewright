...
description: Skriv dine egne programtillegg for å utvide Codewright.
---

Plugins lar deg utvide Codewright ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Codewright.
...
- `.codewright/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/codewright/node_modules/`.
...
1. Global konfigurasjon (`~/.config/codewright/codewright.json`)
2. Prosjektkonfigurasjon (`codewright.json`)
3. Global plugin-katalog (`~/.config/codewright/plugins/`)
4. Prosjektpluginkatalog (`.codewright/plugins/`)
...
Codewright kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Codewright SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Codewright.
...
:::note
Hvis du bruker Codewright-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Codewright fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Codewright:
...
`tool`-hjelperen lager et tilpasset verktøy som Codewright kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Codewright sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://codewright.dev/docs/sdk) for detaljer.
