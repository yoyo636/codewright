...
Du kan legge til eksterne verktøy i Codewright ved å bruke _Model Context Protocol_ eller MCP. Codewright støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Codewright Config](https://codewright.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="codewright.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/codewright`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="codewright.jsonc" {15}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
Codewright håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Codewright:
...
Hvis serveren krever autentisering, vil Codewright be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `codewright mcp auth <server-name>`.
...
```json title="codewright.json" {7-11}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
codewright mcp auth my-oauth-server
```
...
```bash
codewright mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Codewright lagre tokens sikkert i `~/.local/share/codewright/mcp-auth.json`.
...
codewright mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Codewright, sammen med innebygde verktøy. Så du kan administrere dem gjennom Codewright-konfigurasjonen som alle andre verktøy.
...
```json title="codewright.json" {11, 14-18}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="codewright.json" {4-8}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "sentry": {
...
```bash
codewright mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Codewright til Sentry-kontoen din.
...
```json title="codewright.json" {4-7}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "context7": {
...
```json title="codewright.json" {7-9}
{
  "$schema": "https://codewright.dev/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver Codewright-samtale oppført som en separat økt.
