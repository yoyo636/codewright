---
title: Tjener
description: Samhandle med Codewright-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `codewright serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en Codewright-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `codewright.local` |
...
Sett `CODEWRIGHT_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `codewright` som standard, eller sett `CODEWRIGHT_SERVER_USERNAME` for å overstyre det. Dette gjelder både `codewright serve` og `codewright web`.
...
Når du kjører `codewright` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk Codewright-serveren til å samhandle med Codewright programmatisk.
:::

Denne arkitekturen lar Codewright støtte flere klienter og lar deg samhandle med Codewright programmatisk.

Du kan kjøre `codewright serve` for å starte en frittstående server. Hvis du har
Codewright TUI kjører, vil `codewright serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Codewright [IDE](/docs/ide) plugins.
...
codewright-serveren viser følgende APIer.
