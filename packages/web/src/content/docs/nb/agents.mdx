...
Det er to typer agenter i Codewright; primære agenter og underagenter.
...
Codewright kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Codewright kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Codewright kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `codewright.json`:
...
- Per prosjekt: `.codewright/agents/`
...
Hvis ingen temperatur er spesifisert, bruker Codewright modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale Codewright-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i Codewright-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Codewright Zen](/docs/zen), vil du bruke `codewright/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `codewright models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/codewright).
:::
