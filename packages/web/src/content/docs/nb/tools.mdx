...
Verktøy lar LLM utføre handlinger i kodebasen din. Codewright kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i Codewright.
...
:::note
Dette verktøyet er bare tilgjengelig når `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (eller `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker Codewright-leverandøren eller når miljøvariabelen `CODEWRIGHT_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter Codewright:
