...
You can add external tools to Codewright using the _Model Context Protocol_, or MCP. Codewright supports both local and remote servers.
...
You can define MCP servers in your [Codewright Config](https://codewright.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/codewright` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Codewright automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Codewright will:
...
If the server requires authentication, Codewright will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `codewright mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Codewright will store the tokens securely in `~/.local/share/codewright/mcp-auth.json`.
...
Your MCPs are available as tools in Codewright, alongside built-in tools. So you can manage them through the Codewright config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Codewright to your Sentry account.
