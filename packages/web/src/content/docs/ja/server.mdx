---
title: サーバー
description: HTTP 経由で Codewright サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codewright serve` コマンドは、Codewright クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `codewright.local` |
...
HTTP 基本認証でサーバーを保護するには、`CODEWRIGHT_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `codewright` になるか、`CODEWRIGHT_SERVER_USERNAME` を設定してオーバーライドします。これは、`codewright serve` と `codewright web` の両方に当てはまります。
...
`codewright` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
Codewright サーバーを使用して、プログラムで Codewright と対話します。
:::

`codewright serve` を実行してスタンドアロンサーバーを起動できます。すでに Codewright TUI を実行している場合でも、`codewright serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、Codewright [IDE](/docs/ide) プラグインによって使用されます。
