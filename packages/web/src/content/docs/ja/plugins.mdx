---
title: プラグイン
description: Codewright を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Codewright を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Codewright のデフォルトの動作を変更したりできます。
...
- `.codewright/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/codewright/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/codewright/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/codewright/codewright.json`)
2. プロジェクト設定 (`codewright.json`)
3. グローバルプラグインディレクトリ (`~/.config/codewright/plugins/`)
4. プロジェクトプラグインディレクトリ (`.codewright/plugins/`)
...
Codewright は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Codewright SDK クライアント。
...
ここでは、Codewright を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://codewright.dev/docs/sdk) を参照してください。
...
Codewright が `.env` ファイルを読み取らないようにします。
...
プラグインは Codewright にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Codewright が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Codewright で使用できます。
