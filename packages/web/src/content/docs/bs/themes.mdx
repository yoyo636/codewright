...
U Codewright mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Codewright koristi nasu `codewright` temu.
...
Codewright dolazi sa vise ugradenih tema.
...
- Zele da Codewright odgovara izgledu njihovog terminala
...
Codewright podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/codewright/themes/*.json` ili `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.codewright/themes/*.json`
4. **Trenutni radni direktorij** - `./.codewright/themes/*.json`
