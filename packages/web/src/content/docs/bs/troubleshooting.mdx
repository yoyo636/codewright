...
Da biste otklonili probleme s Codewright, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codewright\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `codewright --log-level DEBUG`.

---

## Pohrana

Codewright pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/codewright/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\codewright`
...
Codewright Desktop pokreće lokalni Codewright server (`codewright-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Codewright` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/codewright/codewright.jsonc` (ili `~/.config/codewright/codewright.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/codewright/codewright.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codewright\codewright.jsonc`
...
Codewright također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/codewright/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\codewright\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Codewright mogao ponovo izgraditi.

1. Potpuno zatvorite Codewright Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/codewright`
- **Linux**: obrišite `~/.cache/codewright` (ili pokrenite `rm -rf ~/.cache/codewright`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\codewright`

3. Ponovo pokrenite Codewright Desktop.

---

### Rješavanje problema sa vezom na serveru

Codewright Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `codewright.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CODEWRIGHT_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CODEWRIGHT_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Codewright Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Codewright karakteristikama.

---

### Obavještenja se ne prikazuju

Codewright Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Codewright u postavkama vašeg OS-a, i
...
1. Zatvorite Codewright Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Codewright Desktop aplikacije):

- `codewright.settings.dat` (URL zadanog servera za desktop)
- `codewright.global.dat` i `codewright.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s Codewright:
...
Da saznate kojim modelima imate pristup, pokrenite `codewright models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\codewright`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Codewright dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\codewright`

2. Ponovo pokrenite Codewright da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Codewright da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Codewright će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
