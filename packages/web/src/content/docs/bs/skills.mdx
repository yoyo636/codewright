...
Agent skills omogucavaju Codewright da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Codewright pretrazuje ove lokacije:

- Konfiguracija projekta: `.codewright/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/codewright/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Codewright ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.codewright/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.codewright/skills/git-release/SKILL.md` ovako:
...
Codewright navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `codewright.json`:
...
**Za ugradene agente** (u `codewright.json`):
