...
Možete dodati vanjske alate u Codewright koristeći _Model Context Protocol_, ili MCP. Codewright podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [Codewright Config](https://codewright.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/codewright`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
Codewright automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, Codewright će:
...
Ako server zahtijeva autentifikaciju, Codewright će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `codewright mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, Codewright sigurno čuva tokene u `~/.local/share/codewright/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u Codewright, zajedno s ugrađenim alatima. Možete njima upravljati kroz Codewright konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete Codewright sa vašim Sentry nalogom.
