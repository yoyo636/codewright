---
title: Server
description: Komunicirajte s codewright serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `codewright serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji codewright klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `codewright.local` |
...
Postavite `CODEWRIGHT_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `codewright`, ili postavite `CODEWRIGHT_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `codewright serve` i za `codewright web`.
...
Kada pokrenete `codewright`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite codewright server za programsku interakciju sa codewright-om.
:::

Ova arhitektura omogucava codewright podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `codewright serve` da startate standalone server. Ako je codewright TUI vec pokrenut, `codewright serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Codewright [IDE](/docs/ide) pluginovi.
...
codewright server izlaže sljedece API-je.
