...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Codewright dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Codewright.
...
:::note
Ovaj alat je dostupan samo kada je `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (ili `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Codewright provajdera ili kada je varijabla `CODEWRIGHT_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Codewright:
