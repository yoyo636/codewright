---
title: Konfiguracija
description: Korištenje Codewright JSON konfiguracije.
---

Možete konfigurirati Codewright koristeći JSON konfiguracijski fajl.

---

## Format

Codewright podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/codewright`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/codewright/codewright.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CODEWRIGHT_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`codewright.json` u projektu) - postavke specifične za projekat
5. **`.codewright` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CODEWRIGHT_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.codewright` i `~/.config/codewright` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/codewright` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Codewright konfiguraciju u `~/.config/codewright/codewright.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/codewright/tui.json`.
...
Dodajte `codewright.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Codewright pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CODEWRIGHT_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CODEWRIGHT_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.codewright` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.codewright`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`codewright.dev/config.json`**](https://codewright.dev/config.json).

TUI konfiguracija koristi [**`codewright.dev/tui.json`**](https://codewright.dev/tui.json).
...
Koristite `CODEWRIGHT_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `codewright.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `codewright serve` i `codewright web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Codewright server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `codewright.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Codewright konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Codewright pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/codewright/agents/` ili `.codewright/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Codewright će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`codewright run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/codewright/commands/` ili `.codewright/commands/`. [Saznajte više ovdje](/docs/commands).
...
Codewright će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Codewright **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Codewright sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.codewright/plugins/` ili `~/.config/codewright/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Codewright da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
