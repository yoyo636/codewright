---
title: LSP serveri
description: Codewright se integriše sa vašim LSP serverima.
---

Codewright se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

Codewright dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `CODEWRIGHT_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i codewright otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj codewright konfiguraciji.
