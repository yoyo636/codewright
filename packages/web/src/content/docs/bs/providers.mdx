---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

Codewright koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Codewright konfiguraciji.
...
u `~/.local/share/codewright/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Codewright
config.
...
Codewright Zen je lista modela koje je obezbedio Codewright tim koji su bili
testirano i potvrđeno da dobro radi sa Codewright. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Codewright Zen.
:::
...
Radi kao i svaki drugi provajder u Codewright i potpuno je opcionalan za korištenje.

---

## Codewright Go

Codewright Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Codewright tim i koji su testirani i verificirani da dobro rade s Codewright-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Codewright Go` i idite na [codewright.dev/auth](https://codewright.dev/zen).
...
Da biste koristili Amazon Bedrock s Codewright:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete codewright:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `codewright.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Codewright.
:::
...
Možete konfigurirati codewright za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi codewright ispravno radio.
   :::
...
Također možete dodati modele kroz svoju codewright konfiguraciju.
...
da pohrani token u codewright auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Codewright koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Codewright
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`codewright.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth).

##### Konfiguracija

Prilagodite putem `codewright.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s codewright:
...
Za korištenje Google Vertex AI s Codewright:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće codewright.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [codewright-helicone-session](https://github.com/H2Shami/codewright-helicone-session) da automatski prijavite svaki Codewright razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Codewright razgovor naveden kao zasebna sesija.
...
Možete konfigurirati codewright za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati codewright za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati codewright za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Codewright. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/codewright) za detalje.
:::
...
Da biste koristili Ollama Cloud s Codewright:
...
3. Kopirajte API ključ za korištenje u Codewright.
...
6. **Važno**: Prije upotrebe modela oblaka u Codewright, morate lokalno povući informacije o modelu:
...
Codewright Zen je lista testiranih i verifikovanih modela koju je obezbedio Codewright tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Codewright Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Codewright Zen**.

```txt
   /connect
```

3. Unesite svoj Codewright API ključ.
...
Također možete dodati dodatne modele putem vaše codewright konfiguracije.
...
5. Također ih možete prilagoditi putem vaše codewright konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa Codewrightom:
...
Također možete prilagoditi modele kroz svoju codewright konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s codewright-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `codewright.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Codewright da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `codewright auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Codewright konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj codewright konfiguraciji.
