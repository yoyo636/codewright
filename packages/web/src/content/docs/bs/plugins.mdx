---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Codewright.
---

Dodaci vam omogućavaju da proširite Codewright spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Codewright.
...
- `.codewright/plugins/` - Dodaci na nivou projekta
- `~/.config/codewright/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/codewright/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/codewright/codewright.json`)
2. Konfiguracija projekta (`codewright.json`)
3. Globalni direktorij dodataka (`~/.config/codewright/plugins/`)
4. Direktorij dodataka projekta (`.codewright/plugins/`)
...
Codewright pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Codewright SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Codewright.
...
Spriječite codewright da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u codewright:
...
Pomoćnik `tool` kreira prilagođeni alat koji codewright može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za codewright zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://codewright.dev/docs/sdk) za detalje.
