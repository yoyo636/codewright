...
يمكنك إضافة أدوات خارجية إلى Codewright باستخدام _Model Context Protocol_، أو MCP. يدعم Codewright خوادم محلية وبعيدة.
...
يمكنك تعريف خوادم MCP في [إعدادات Codewright](https://codewright.dev/docs/config/) ضمن `mcp`. أضف كل خادم MCP باسم فريد. ويمكنك الإشارة إلى ذلك الخادم بالاسم عند توجيه الطلب إلى الـ LLM.
...
يمكن للمنظمات توفير خوادم MCP الافتراضية عبر نقطة النهاية `.well-known/codewright` الخاصة بها. قد تكون هذه الخوادم معطّلة افتراضيا، مما يتيح للمستخدمين تفعيل ما يحتاجونه فقط.
...
يتولى Codewright تلقائيا معالجة مصادقة OAuth لخوادم MCP البعيدة. عندما يتطلب خادم ما المصادقة، سيقوم Codewright بما يلي:
...
إذا كان الخادم يتطلب المصادقة، فسيطلب منك Codewright المصادقة عند أول محاولة لاستخدامه. وإذا لم يحدث ذلك، يمكنك [تشغيل التدفق يدويا](#authenticating) باستخدام `codewright mcp auth <server-name>`.
...
سيفتح أمر `mcp auth` متصفحك للتفويض. بعد إكمال التفويض، سيخزّن Codewright الرموز بشكل آمن في `~/.local/share/codewright/mcp-auth.json`.
...
تتوفر خوادم MCP لديك كأدوات داخل Codewright إلى جانب الأدوات المضمنة. لذا يمكنك إدارتها عبر إعدادات Codewright مثل أي أداة أخرى.
...
سيؤدي ذلك إلى فتح نافذة متصفح لإكمال تدفق OAuth وربط Codewright بحساب Sentry الخاص بك.
