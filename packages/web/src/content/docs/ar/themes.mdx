...
مع Codewright يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم Codewright سمتنا `codewright`.
...
يأتي Codewright مع عدة سمات مدمجة.
...
- يريدون أن يطابق Codewright مظهر terminal لديهم
...
يدعم Codewright نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/codewright/themes/*.json` أو `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.codewright/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.codewright/themes/*.json`
