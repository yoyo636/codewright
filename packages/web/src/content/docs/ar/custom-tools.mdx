...
description: أنشئ أدوات يمكن لـ LLM استدعاؤها في codewright.
---

الأدوات المخصصة هي دوال تنشئها يمكن لـ LLM استدعاؤها أثناء المحادثات. تعمل جنبا إلى جنب مع [الأدوات المدمجة](/docs/tools) في codewright مثل `read` و`write` و`bash`.
...
- محليا بوضعها في مجلد `.codewright/tools/` داخل مشروعك.
- أو عالميا بوضعها في `~/.config/codewright/tools/`.
