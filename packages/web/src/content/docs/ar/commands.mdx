...
أنشئ الملف `.codewright/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Codewright أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Codewright:
...
- على مستوى النظام: `~/.config/codewright/commands/`
- لكل مشروع: `.codewright/commands/`
...
يتضمن codewright عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
