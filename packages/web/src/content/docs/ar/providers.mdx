---
title: المزوّدون
description: استخدام أي موفّر LLM في Codewright.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Codewright ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Codewright.
...
في `~/.local/share/codewright/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Codewright.
...
Codewright Zen هي قائمة نماذج يوفّرها فريق Codewright وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Codewright. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Codewright Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر codewright، ثم انتقل إلى [codewright.dev/auth](https://codewright.dev/auth).
...
يعمل مثل أي مزوّد آخر في Codewright واستخدامه اختياري بالكامل.

---

## Codewright Go

Codewright Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق Codewright، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Codewright.

1. شغّل الأمر `/connect` في TUI، واختر `Codewright Go`، ثم انتقل إلى [codewright.dev/auth](https://codewright.dev/zen).
...
لاستخدام Amazon Bedrock مع Codewright:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل codewright:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `codewright.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Codewright غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين codewright لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل codewright بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات codewright.
...
تخزين الرمز في مساحة تخزين مصادقة codewright.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Codewright نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Codewright
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`codewright.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [codewright-gitlab-auth](https://www.npmjs.com/package/codewright-gitlab-auth).

##### التهيئة

خصّص عبر `codewright.json`:
...
لاستخدام اشتراك GitHub Copilot مع codewright:
...
لاستخدام Google Vertex AI مع Codewright:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر codewright، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [codewright-helicone-session](https://github.com/H2Shami/codewright-helicone-session) لتسجيل كل محادثة Codewright تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Codewright مدرجة كجلسة مستقلة.
...
يمكنك ضبط codewright لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط codewright لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط codewright لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Codewright. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/codewright) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Codewright:
...
3. انسخ مفتاح API لاستخدامه في Codewright.
...
6. **مهم**: قبل استخدام نماذج السحابة في Codewright، يجب سحب معلومات النموذج محليا:
...
Codewright Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Codewright. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Codewright Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Codewright Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Codewright.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات codewright.
...
5. يمكنك أيضا تخصيصها عبر إعدادات codewright. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Codewright:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات codewright. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع codewright. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `codewright.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Codewright بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `codewright auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات codewright وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات codewright.
