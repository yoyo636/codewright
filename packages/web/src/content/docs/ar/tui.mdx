---
title: TUI
description: استخدام واجهة المستخدم TUI في Codewright.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

يوفّر Codewright واجهة terminal تفاعلية (TUI) للعمل على مشاريعك باستخدام LLM.

يؤدي تشغيل Codewright إلى بدء واجهة TUI للدليل الحالي.
...
عند استخدام واجهة Codewright في terminal، يمكنك كتابة `/` متبوعة باسم أمر لتنفيذ الإجراءات بسرعة. مثلا:
...
أضف موفّرا إلى Codewright. يتيح لك اختيار أحد الموفّرين المتاحين وإضافة مفاتيح API الخاصة بهم.
...
اخرج من Codewright. _الأسماء المستعارة_: `/quit`, `/q`
...
هذا منفصل عن `codewright.json`، الذي يضبط سلوك الخادم ووقت التشغيل.
...
- `leader_timeout` - يتحكم في مدة انتظار Codewright بعد الضغط على leader key. القيمة الافتراضية `2000`.
...
- `sound_pack` - sound pack ID المراد استخدامه. القيمة الافتراضية `codewright.default`.
