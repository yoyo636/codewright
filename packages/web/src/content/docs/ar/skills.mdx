...
تتيح مهارات الوكيل لـ Codewright اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Codewright في هذه المواقع:

- إعدادات المشروع: `.codewright/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/codewright/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Codewright صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.codewright/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/codewright/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.codewright/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Codewright المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `codewright.json`:
...
**للوكلاء المدمجين** (في `codewright.json`):
