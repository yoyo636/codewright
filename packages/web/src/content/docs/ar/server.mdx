---
title: الخادم
description: تفاعل مع خادم codewright عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `codewright serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل codewright استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `codewright.local` |
...
عيّن `CODEWRIGHT_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `codewright`، أو عيّن `CODEWRIGHT_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `codewright serve` و `codewright web`.
...
عند تشغيل `codewright` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم codewright للتفاعل مع codewright برمجيا.
:::

تتيح هذه البنية لـ codewright دعم عدة عملاء وتمكّنك من التفاعل مع codewright برمجيا.

يمكنك تشغيل `codewright serve` لبدء خادم مستقل. إذا كانت واجهة codewright في terminal (TUI)
قيد التشغيل، فسيبدأ `codewright serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Codewright لـ [IDE](/docs/ide).
...
يعرض خادم codewright واجهات API التالية.
