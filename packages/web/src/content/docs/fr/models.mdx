...
Les fournisseurs les plus populaires sont préchargés par défaut. Si vous avez ajouté les informations d'identification d'un fournisseur via la commande `/connect`, elles seront disponibles lorsque vous démarrerez Codewright.
...
Voici plusieurs modèles qui fonctionnent bien avec Codewright, sans ordre particulier. (Cette liste n’est pas exhaustive et n’est pas nécessairement à jour) :
...
Pour définir l'un d'entre eux comme modèle par défaut, vous pouvez définir la clé `model` dans votre configuration Codewright.

```json title="codewright.json" {3}
{
...
Ici, l'identifiant complet est `provider_id/model_id`. Par exemple, si vous utilisez [Codewright Zen](/docs/zen), vous utiliserez `codewright/gpt-5.1-codex` pour GPT 5.1 Codex.
...
```jsonc title="codewright.jsonc" {7-12,19-24}
{
  "$schema": "https://codewright.dev/config.json",
  "provider": {
    "openai": {
...
```jsonc title="codewright.jsonc" {6-21}
{
  "$schema": "https://codewright.dev/config.json",
  "provider": {
    "codewright": {
...
De nombreux modèles prennent en charge plusieurs variantes avec différentes configurations. Codewright est livré avec des variantes par défaut intégrées pour les fournisseurs populaires.

### Variantes intégrées

Codewright est livré avec des variantes par défaut pour de nombreux fournisseurs :
...
```jsonc title="codewright.jsonc" {7-18}
{
  "$schema": "https://codewright.dev/config.json",
  "provider": {
    "openai": {
...
Lorsque Codewright démarre, il recherche les modèles dans l'ordre de priorité suivant :
...
2. La liste des modèles dans la configuration Codewright.
