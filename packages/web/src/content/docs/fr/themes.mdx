...
Avec Codewright, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Codewright utilise notre propre thème `codewright`.
...
Codewright est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Codewright corresponde à l'apparence de leur terminal
...
Codewright prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/codewright/themes/*.json` ou `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.codewright/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.codewright/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/codewright/themes
vim ~/.config/codewright/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .codewright/themes
vim .codewright/themes/my-theme.json
```
...
{
  "$schema": "https://codewright.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
