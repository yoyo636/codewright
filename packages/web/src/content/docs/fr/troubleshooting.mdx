...
Pour déboguer les problèmes avec Codewright, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\codewright\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `codewright --log-level DEBUG`.

---

## Stockage

codewright stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/codewright/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\codewright`
...
Codewright Desktop exécute un serveur Codewright local (le side-car `codewright-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Codewright` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/codewright/codewright.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\codewright\codewright.jsonc`
...
Codewright peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/codewright/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\codewright\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Codewright puisse le reconstruire.

1. Quittez complètement Codewright Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/codewright`
- **Linux** : supprimez `~/.cache/codewright` (ou exécutez `rm -rf ~/.cache/codewright`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\codewright`

3. Redémarrez le bureau Codewright.

---

### Résoudre les problèmes de connexion au serveur

Codewright Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `codewright.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `CODEWRIGHT_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `CODEWRIGHT_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Codewright Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Codewright.

---

### Les notifications ne s'affichent pas

Codewright Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Codewright dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Codewright.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Codewright Desktop) :

- `codewright.settings.dat` (serveur de bureau par défaut URL)
- `codewright.global.dat` et `codewright.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Codewright :
...
3. Assurez-vous d'avoir la dernière version avec `codewright upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `codewright models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\codewright`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. codewright installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\codewright`

2. Redémarrez codewright pour réinstaller les derniers packages du fournisseur

Cela forcera codewright à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
codewright détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
