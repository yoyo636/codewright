---
title: Serveurs LSP
description: Codewright s'intègre à vos serveurs LSP.
---

Codewright peut s'intégrer aux serveurs Language Server Protocol (LSP) pour utiliser les diagnostics comme retour pour l'agent.

---

## Serveurs intégrés

Codewright est livré avec plusieurs serveurs LSP intégrés pour les langages populaires :
...
:::note
Vous pouvez désactiver les téléchargements automatiques du serveur LSP en définissant la variable d'environnement `CODEWRIGHT_DISABLE_LSP_DOWNLOAD` sur `true`.
:::
...
Lorsque LSP est activé et qu'codewright ouvre un fichier, il :
...
Vous pouvez activer et personnaliser les serveurs LSP via la section `lsp` de votre configuration codewright.
...
```json title="codewright.json" {5-7}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "rust": {
...
```json title="codewright.json" {5-9}
{
  "$schema": "https://opencode.ai/config.json",
  "lsp": {
    "typescript": {
