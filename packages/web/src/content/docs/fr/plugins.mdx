---
title: Plugins
description: Écrivez vos propres plugins pour étendre Codewright.
---

Les plugins vous permettent d'étendre Codewright en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Codewright.
...
- `.codewright/plugins/` - Plugins au niveau du projet
- `~/.config/codewright/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/codewright/node_modules/`.
...
1. Configuration globale (`~/.config/codewright/codewright.json`)
2. Configuration du projet (`codewright.json`)
3. Répertoire global des plugins (`~/.config/codewright/plugins/`)
4. Répertoire des plugins du projet (`.codewright/plugins/`)
...
Codewright exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK codewright pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre codewright.
...
:::note
Si vous utilisez l'application de bureau Codewright, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez codewright de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à codewright :
...
L'assistant `tool` crée un outil personnalisé que codewright peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour codewright aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://codewright.dev/docs/sdk) pour plus de détails.
