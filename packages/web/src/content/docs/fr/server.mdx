---
title: Serveur
description: Interagissez avec le serveur codewright via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `codewright serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client codewright peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `codewright.local` |
...
Définissez `CODEWRIGHT_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `codewright`, ou définissez `CODEWRIGHT_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `codewright serve` et à `codewright web`.
...
Lorsque vous exécutez `codewright`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur codewright pour interagir avec codewright par programmation.
:::

Cette architecture permet à codewright de prendre en charge plusieurs clients et vous permet d'interagir avec codewright par programme.

Vous pouvez exécuter `codewright serve` pour démarrer un serveur autonome. Si vous avez le
codewright TUI en cours d'exécution, `codewright serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins Codewright [IDE](/docs/ide).
...
Le serveur codewright expose les API suivantes.
