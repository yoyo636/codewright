---
title: SDK
description: Client JS de type sécurisé pour le serveur codewright.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK codewright JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler codewright par programme.
...
Créez une instance de codewright :

```javascript
import { createCodewright } from "@codewright-ai/sdk"

const { client } = await createCodewright()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `codewright.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${codewright.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de codewright, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@codewright-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*codewright" },
})
