---
title: Server
description: Interact with codewright server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `codewright serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an codewright client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `codewright.local` |
...
Set `CODEWRIGHT_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `codewright`, or set `CODEWRIGHT_SERVER_USERNAME` to override it. This applies to both `codewright serve` and `codewright web`.
...
When you run `codewright` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the codewright server to interact with codewright programmatically.
:::

This architecture lets codewright support multiple clients and allows you to interact with codewright programmatically.

You can run `codewright serve` to start a standalone server. If you have the
codewright TUI running, `codewright serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Codewright [IDE](/docs/ide) plugins.
...
The codewright server exposes the following APIs.
