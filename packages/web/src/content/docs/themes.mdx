...
With Codewright you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Codewright uses our own `codewright` theme.
...
Codewright comes with several built-in themes.
...
- Want Codewright to match their terminal's appearance
...
Codewright supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/codewright/themes/*.json` or `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.codewright/themes/*.json`
4. **Current working directory** - Defined in `./.codewright/themes/*.json`
