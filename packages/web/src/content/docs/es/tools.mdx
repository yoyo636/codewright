...
Las herramientas permiten que LLM realice acciones en su código base. Codewright viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Codewright.
...
:::note
Esta herramienta solo está disponible cuando `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (o `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Codewright o cuando la variable de entorno `CODEWRIGHT_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Codewright:

```bash
CODEWRIGHT_ENABLE_EXA=1 codewright
```
