---
title: Complementos
description: Escriba sus propios complementos para extender Codewright.
---

Los complementos le permiten extender Codewright al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Codewright.
...
- `.codewright/plugins/` - Complementos a nivel de proyecto
- `~/.config/codewright/plugins/` - Complementos globales
...
```json title="codewright.json"
{
  ...
  "plugin": ["codewright-helicone-session", "codewright-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/codewright/node_modules/`.
...
1. Configuración global (`~/.config/codewright/codewright.json`)
2. Configuración del proyecto (`codewright.json`)
3. Directorio global de complementos (`~/.config/codewright/plugins/`)
4. Directorio de complementos del proyecto (`.codewright/plugins/`)
...
```json title=".codewright/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
Codewright ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".codewright/plugins/my-plugin.ts"
...
```js title=".codewright/plugins/example.js"
...
- `client`: Un cliente SDK codewright para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar codewright.
...
```js title=".codewright/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "codewright"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio Codewright, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que codewright lea archivos `.env`:

```javascript title=".codewright/plugins/env-protection.js"
...
```javascript title=".codewright/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a codewright:

```ts title=".codewright/plugins/custom-tools.ts"
import { type Plugin, tool } from "@codewright-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que codewright puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para codewright junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://codewright.dev/docs/sdk) para obtener más detalles.
...
```ts title=".codewright/plugins/compaction.ts"
import type { Plugin } from "@codewright-ai/plugin"
...
```ts title=".codewright/plugins/custom-compaction.ts"
import type { Plugin } from "@codewright-ai/plugin"
