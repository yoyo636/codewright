...
Las habilidades del agente permiten a Codewright descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Codewright busca estas ubicaciones:

- Configuración del proyecto: `.codewright/skills/<name>/SKILL.md`
- Configuración global: `~/.config/codewright/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Codewright sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.codewright/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.codewright/skills/git-release/SKILL.md` así:
...
Codewright enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `codewright.json`:
...
**Para agentes integrados** (en `codewright.json`):
