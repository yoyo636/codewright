...
Con Codewright puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, Codewright usa nuestro propio tema `codewright`.
...
Codewright viene con varios temas integrados.
...
- Quiere que Codewright coincida con la apariencia de su terminal
...
Codewright admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/codewright/themes/*.json` o `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.codewright/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.codewright/themes/*.json`
