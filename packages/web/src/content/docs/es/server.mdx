---
title: Servidor
description: Interactuar con el servidor codewright a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `codewright serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente codewright puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `codewright.local` |
...
Configure `CODEWRIGHT_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `codewright`, o configure `CODEWRIGHT_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `codewright serve` como a `codewright web`.
...
Cuando ejecuta `codewright`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor codewright para interactuar con codewright mediante programación.
:::

Esta arquitectura permite que codewright admita múltiples clientes y le permite interactuar con codewright mediante programación.

Puede ejecutar `codewright serve` para iniciar un servidor independiente. Si tienes el
codewright TUI ejecutándose, `codewright serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Codewright [IDE](/docs/ide).
...
El servidor codewright expone las siguientes API.
