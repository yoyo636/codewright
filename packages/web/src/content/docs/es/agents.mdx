...
Hay dos tipos de agentes en Codewright; agentes primarios y subagentes.
...
Codewright viene con dos agentes principales integrados, **Build** y **Plan**. Bien
...
Codewright viene con tres subagentes integrados, **General**, **Explore** y **Scout**. Veremos esto a continuación.

---

## Integrados

Codewright viene con dos agentes primarios integrados y tres subagentes integrados.
...
Configure agentes en su archivo de configuración `codewright.json`:
...
- Por proyecto: `.codewright/agents/`
...
Si no se especifica ninguna temperatura, Codewright utiliza valores predeterminados específicos del modelo; normalmente 0 para la mayoría de los modelos, 0,55 para los modelos Qwen.
...
Esta ruta es relativa a donde se encuentra el archivo de configuración. Entonces esto funciona tanto para la configuración global Codewright como para la configuración específica del proyecto.
...
El ID del modelo en su configuración Codewright usa el formato `provider/model-id`. Por ejemplo, si usa [Codewright Zen](/docs/zen), usaría `codewright/gpt-5.1-codex` para GPT 5.1 Codex.
...
:::tip
Ejecute `codewright models` para ver una lista de los modelos disponibles.
:::
...
:::tip
¿Tiene un agente que le gustaría compartir? [Enviar un PR](https://github.com/anomalyco/codewright).
:::
