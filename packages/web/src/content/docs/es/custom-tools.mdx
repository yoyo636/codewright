...
description: Cree herramientas que LLM pueda llamar en codewright.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de codewright como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.codewright/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/codewright/tools/`.
...
```ts title=".codewright/tools/database.ts" {1}
...
```ts title=".codewright/tools/math.ts"
import { tool } from "@codewright-ai/plugin"
...
```ts title=".codewright/tools/bash.ts"
import { tool } from "@codewright-ai/plugin"
...
```ts title=".codewright/tools/project.ts" {8}
import { tool } from "@codewright-ai/plugin"
...
```python title=".codewright/tools/add.py"
import sys
...
```ts title=".codewright/tools/python-add.ts" {10}
import { tool } from "@codewright-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".codewright/tools/add.py")
