---
title: Server
description: Interagisci con il server codewright via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `codewright serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client codewright.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `codewright.local` |
...
Imposta `CODEWRIGHT_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `codewright`, oppure imposta `CODEWRIGHT_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `codewright serve` sia per `codewright web`.
...
Quando esegui `codewright` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server codewright per interagire con codewright in modo programmatico.
:::

Questa architettura permette a codewright di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `codewright serve` per avviare un server standalone. Se la TUI di codewright e gia in esecuzione, `codewright serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di Codewright.
...
Il server codewright espone le seguenti API.
