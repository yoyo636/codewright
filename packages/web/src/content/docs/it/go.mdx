...
Codewright Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in Codewright. Ti abboni a Codewright Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare Codewright.
...
Codewright Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

Codewright Go funziona come qualsiasi altro provider in Codewright.

1. Accedi a **<a href={console}>Codewright Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `Codewright Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a Codewright Go.
:::
...
Codewright Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua Codewright config
utilizza il formato `codewright-go/<model-id>`. Ad esempio, per Kimi K3, useresti
`codewright-go/kimi-k3` nella tua configurazione.
...
Abbiamo creato Codewright Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con Codewright.
