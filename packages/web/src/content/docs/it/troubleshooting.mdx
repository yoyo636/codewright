...
Per diagnosticare problemi con Codewright, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\codewright\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `codewright --log-level DEBUG`.

---

## Archiviazione

codewright salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/codewright/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\codewright`
...
Codewright Desktop esegue in background un server locale di Codewright (il sidecar `codewright-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `Codewright` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/codewright/codewright.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\codewright\codewright.jsonc`
...
Codewright puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/codewright/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\codewright\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che Codewright possa ricostruirla.

1. Chiudi completamente Codewright Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/codewright`
- **Linux**: elimina `~/.cache/codewright` (oppure esegui `rm -rf ~/.cache/codewright`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\codewright`

3. Riavvia Codewright Desktop.

---

### Risolvi problemi di connessione al server

Codewright Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `codewright.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `CODEWRIGHT_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `CODEWRIGHT_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, Codewright Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di Codewright.

---

### Notifiche non visualizzate

Codewright Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per Codewright nelle impostazioni del sistema operativo, e
...
1. Chiudi Codewright Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app Codewright Desktop):

- `codewright.settings.dat` (desktop default server URL)
- `codewright.global.dat` e `codewright.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con Codewright:
...
Per capire a quali modelli hai accesso, esegui `codewright models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\codewright`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. codewright installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\codewright`

2. Riavvia codewright per reinstallare i pacchetti provider piu recenti

Questo forzera codewright a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
codewright rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
