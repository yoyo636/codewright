...
Puoi aggiungere strumenti esterni a Codewright usando il _Model Context Protocol_, o MCP. Codewright supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [Codewright Config](https://codewright.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/codewright`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
Codewright gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, Codewright:
...
Se il server richiede autenticazione, Codewright ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `codewright mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, Codewright memorizzera' i token in modo sicuro in `~/.local/share/codewright/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in Codewright insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione Codewright come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare Codewright al tuo account Sentry.
