...
Gli strumenti permettono all'LLM di eseguire azioni nel tuo codebase. Codewright include un set di strumenti integrati, ma puoi estenderlo con [strumenti personalizzati](/docs/custom-tools) o con [server MCP](/docs/mcp-servers).
...
Ecco tutti gli strumenti integrati disponibili in Codewright.
...
:::note
Questo strumento e' disponibile solo quando `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (oppure `CODEWRIGHT_EXPERIMENTAL=true`).
:::
...
:::note
Questo strumento e' disponibile solo quando usi il provider Codewright o quando la variabile d'ambiente `CODEWRIGHT_ENABLE_EXA` e' impostata a un valore truthy (ad esempio `true` o `1`).

Per abilitarlo all'avvio di Codewright:
