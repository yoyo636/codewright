...
Le skill degli agenti permettono a Codewright di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Codewright cerca in queste posizioni:

- Config di progetto: `.codewright/skills/<name>/SKILL.md`
- Config globale: `~/.config/codewright/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Codewright risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.codewright/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.codewright/skills/git-release/SKILL.md` cosi':
...
Codewright elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `codewright.json`:
...
**Per agenti integrati** (in `codewright.json`):
