---
title: Plugin
description: Scrivi plugin per estendere Codewright.
---

I plugin ti permettono di estendere Codewright agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Codewright.
...
- `.codewright/plugins/` - plugin a livello progetto
- `~/.config/codewright/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/codewright/node_modules/`.
...
1. Config globale (`~/.config/codewright/codewright.json`)
2. Config di progetto (`codewright.json`)
3. Directory plugin globale (`~/.config/codewright/plugins/`)
4. Directory plugin di progetto (`.codewright/plugins/`)
...
Codewright esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di codewright per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere codewright.
...
:::note
Se usi l'app desktop di Codewright, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a codewright di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a codewright:
...
L'helper `tool` crea uno strumento personalizzato che codewright può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in codewright insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://codewright.dev/docs/sdk) per i dettagli.
