---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di Codewright.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Codewright fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire Codewright avvia la TUI per la directory corrente.
...
Quando usi la TUI di Codewright, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a Codewright. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da Codewright. _Alias_: `/quit`, `/q`
...
Questo è separato da `codewright.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo Codewright attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `codewright.default`.
