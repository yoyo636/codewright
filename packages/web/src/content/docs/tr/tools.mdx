...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. codewright yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
codewright içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CODEWRIGHT_EXPERIMENTAL_LSP_TOOL=true` (veya `CODEWRIGHT_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca codewright provider kullanılırken veya `CODEWRIGHT_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

codewright başlatırken etkinleştirmek için:
