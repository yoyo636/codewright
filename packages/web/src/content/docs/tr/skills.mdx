...
Ajan becerileri, codewright'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
codewright su konumlari tarar:

- Proje konfigurasyonu: `.codewright/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/codewright/skills/<name>/SKILL.md`
...
Proje yerel yollarinda codewright, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.codewright/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/codewright/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.codewright/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
codewright, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`codewright.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`codewright.json` icinde):
