...
codewright ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak codewright kendi `codewright` temasını kullanır.
...
codewright birden fazla yerleşik temayla gelir.
...
- codewright'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
codewright, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/codewright/themes/*.json` veya `$XDG_CONFIG_HOME/codewright/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.codewright/themes/*.json`
4. **Geçerli çalışma dizini** - `./.codewright/themes/*.json`
