...
`.codewright/commands/test.md` oluştur:
...
codewright işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

codewright [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/codewright/commands/`
- Proje başına: `.codewright/commands/`
...
codewright, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
