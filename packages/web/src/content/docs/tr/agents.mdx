...
codewright'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
codewright, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Codewright, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Codewright iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `codewright.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.codewright/agents/`
...
Sıcaklık belirtilmezse codewright modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global codewright yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
codewright yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Codewright Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `codewright/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `codewright models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/codewright).
:::
