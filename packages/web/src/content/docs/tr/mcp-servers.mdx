...
_Model Bağlam Protokolü_ veya MCP kullanarak codewright'a harici araçlar ekleyebilirsiniz. codewright hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [codewright Config](https://codewright.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/codewright` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
codewright, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde codewright şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa codewright, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `codewright mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından codewright, belirteçleri `~/.local/share/codewright/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra codewright'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi codewright yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve codewright'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
