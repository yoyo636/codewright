...
Codewright Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, Codewright'daki diğer sağlayıcılar gibi çalışır. Codewright Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve Codewright'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Codewright Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Codewright Go, Codewright'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Codewright Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Codewright Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Codewright Go'ya abone olabilir.
:::
...
Codewright Go aşağıdaki limitleri içerir:
...
Codewright yapılandırmanızdaki [model id](/docs/config/#models) formatı `codewright-go/<model-id>` şeklindedir. Örneğin, Kimi K3 için yapılandırmanızda `codewright-go/kimi-k3` kullanmalısınız.
...
Codewright Go'yu şu amaçlarla oluşturduk:
...
4. Codewright ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
