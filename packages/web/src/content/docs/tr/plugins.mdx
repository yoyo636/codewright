---
title: Eklentiler
description: Codewright'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek Codewright'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya Codewright'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.codewright/plugins/` - Proje düzeyinde eklentiler
- `~/.config/codewright/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/codewright/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/codewright/codewright.json`)
2. Project config (`codewright.json`)
3. Global eklenti dizini (`~/.config/codewright/plugins/`)
4. Proje eklenti dizini (`.codewright/plugins/`)
...
Codewright bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Codewright SDK istemcisi.
...
Codewright'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Codewright masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
Codewright'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca Codewright'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, Codewright'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra Codewright'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://codewright.dev/docs/sdk) bakın.
