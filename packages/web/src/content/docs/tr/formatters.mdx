...
description: codewright dile özgü biçimlendiriciler kullanır.
---

codewright, dosyaları dile özgü formatlayıcılar kullanılarak yazıldıktan veya düzenlendikten sonra otomatik olarak formatlar. Bu, oluşturulan kodun projenizin kod stillerine uymasını sağlar.

---

## Yerleşik

codewright, popüler diller ve çerçeveler için çeşitli yerleşik biçimlendiricilerle birlikte gelir. Aşağıda ihtiyaç duyduğu biçimlendiricilerin, desteklenen dosya uzantılarının ve komutların veya yapılandırma seçeneklerinin bir listesi bulunmaktadır.
...
Yani eğer projenizin `package.json` dosyasında `prettier` varsa, codewright bunu otomatik olarak kullanacaktır.

---

## Nasıl çalışır?

codewright bir dosyayı yazdığında veya düzenlediğinde:
...
Biçimlendiricileri codewright yapılandırmanızdaki `formatter` bölümü aracılığıyla özelleştirebilirsiniz.
