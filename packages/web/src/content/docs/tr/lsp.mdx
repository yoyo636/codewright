...
description: Codewright, LSP sunucularınızla bütünleşir.
---

Codewright, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Codewright, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CODEWRIGHT_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve codewright bir dosyayı açtığında:
...
LSP sunucularını, codewright yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
