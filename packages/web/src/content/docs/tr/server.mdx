---
title: Sunucu
description: codewright sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`codewright serve` komutu, codewright istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `codewright.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CODEWRIGHT_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `codewright` değeridir; değiştirmek isterseniz `CODEWRIGHT_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `codewright serve` hem de `codewright web` için geçerlidir.
...
`codewright` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
codewright ile programatik etkileşim için codewright sunucusunu kullanın.
:::

Bu mimari, codewright'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`codewright serve` ile bağımsız bir sunucu başlatabilirsiniz. codewright TUI açıksa, `codewright serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum codewright [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
codewright sunucusu aşağıdaki API'leri sunar.
