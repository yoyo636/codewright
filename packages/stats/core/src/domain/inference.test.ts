...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve codewright route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "codewright")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "codewright")).toBe("openai")
    expect(statProvider("big-pickle", "", "codewright")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "codewright"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep codewright as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "codewright"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "codewright"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep codewright or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "codewright"), country: "US" })).toMatchObject([
