...
import { PromptInput } from "@codewright-ai/schema/prompt-input"
import { Session } from "@codewright-ai/schema/session"
import { Project } from "@codewright-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@codewright-ai/schema/schema"
...
import { SessionEvent } from "@codewright-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Codewright process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Codewright process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
