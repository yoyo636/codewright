import type { TuiPluginApi } from "@codewright-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("codewright.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}codewright.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/codewright/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.codewright/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.codewright/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.codewright/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.codewright/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Codewright from reading sensitive files",
  "Use {highlight}codewright run{/highlight} for non-interactive scripting",
  "Use {highlight}codewright --continue{/highlight} to resume the last session",
  "Use {highlight}codewright run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}codewright serve{/highlight} for headless API access to Codewright",
  "Use {highlight}codewright run --attach{/highlight} to connect to a running server",
  "Run {highlight}codewright upgrade{/highlight} to update to the latest version",
  "Run {highlight}codewright auth list{/highlight} to see all configured providers",
  "Run {highlight}codewright agent create{/highlight} for guided agent creation",
  "Use {highlight}/codewright{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}codewright github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/codewright fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.codewright/themes/{/highlight} directory",
  ...
  "Run {highlight}codewright debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/codewright{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with Codewright Zen for curated, tested models",
