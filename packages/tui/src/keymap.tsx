...
export const CODEWRIGHT_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CODEWRIGHT_MODE_KEY = "codewright.mode"

export const CodewrightKeymapProvider = KeymapProvider
export const useCodewrightKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type CodewrightModeStack = ReturnType<typeof createCodewrightModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, CodewrightModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createCodewrightModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CODEWRIGHT_MODE_KEY, CODEWRIGHT_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CODEWRIGHT_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CODEWRIGHT_MODE_KEY, stack.at(-1)?.mode ?? CODEWRIGHT_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CODEWRIGHT_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CODEWRIGHT_MODE_KEY, undefined)
...
export function useCodewrightModeStack() {
  return getCodewrightModeStack(useCodewrightKeymap())
}

export function getCodewrightModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Codewright mode stack is not registered for this keymap")
  return value
}
...
export function registerCodewrightKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createCodewrightModeStack(keymap)
...
export function useCommandSlashes(): Accessor<readonly CommandSlashEntry[]> {
  const keymap = useCodewrightKeymap()
