import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@codewright-ai/plugin/tui"
...
import type { useCodewrightKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useCodewrightKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getCodewrightModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getCodewrightModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
