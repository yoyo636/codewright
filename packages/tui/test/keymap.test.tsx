...
import { getCodewrightModeStack, CODEWRIGHT_BASE_MODE, CodewrightKeymapProvider, registerCodewrightKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerCodewrightKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when codewright mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getCodewrightModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getCodewrightModeStack(keymap).push("autocomplete")
