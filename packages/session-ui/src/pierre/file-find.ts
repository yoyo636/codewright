...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("codewright-find")
  api.delete("codewright-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("codewright-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("codewright-find", new Highlight(...rest))
    return true
  }
