...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@codewright/acme")).toBe("@codewright/acme")
    expect(Npm.sanitize("@codewright/acme@1.0.0")).toBe("@codewright/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/codewright/acme.git"
    const expected = win ? "acme@git+https_//github.com/codewright/acme.git" : spec
