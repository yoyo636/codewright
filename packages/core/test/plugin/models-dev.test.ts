...
import { ModelsDevPlugin } from "@codewright-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CODEWRIGHT_MODELS_PATH,
          disabled: Flag.CODEWRIGHT_DISABLE_MODELS_FETCH,
        }
        Flag.CODEWRIGHT_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CODEWRIGHT_MODELS_PATH = previous.path
          Flag.CODEWRIGHT_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
