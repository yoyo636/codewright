...
// test/preload.ts pins CODEWRIGHT_MODELS_PATH to a fixture so other tests can
...
// bun process.
const ORIGINAL_MODELS_PATH = Flag.CODEWRIGHT_MODELS_PATH
const ORIGINAL_DISABLE_FETCH = Flag.CODEWRIGHT_DISABLE_MODELS_FETCH
beforeAll(() => {
  Flag.CODEWRIGHT_MODELS_PATH = undefined
  Flag.CODEWRIGHT_DISABLE_MODELS_FETCH = true
})
afterAll(() => {
  Flag.CODEWRIGHT_MODELS_PATH = ORIGINAL_MODELS_PATH
  Flag.CODEWRIGHT_DISABLE_MODELS_FETCH = ORIGINAL_DISABLE_FETCH
})
  ...
  it.live("get() recovers from a corrupted cache file by fetching a fresh catalog", () =>
    Effect.gen(function* () {
      ...
      const result = yield* Effect.acquireUseRelease(
        Effect.sync(() => {
          Flag.CODEWRIGHT_DISABLE_MODELS_FETCH = false
        }),
