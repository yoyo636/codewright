...
import type { IntegrationOAuthMethodRegistration } from "@codewright-ai/plugin/v2/effect/integration"
import { define } from "@codewright-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@codewright-ai/sdk/v2/types"
...
const defaultServer = "https://console.codewright.dev"
const clientID = "codewright-cli"
...
export const CodewrightPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "codewright",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("CodewrightPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("codewright")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load Codewright provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("codewright", (integration) => {
        integration.name = "Codewright"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "codewright", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("codewright")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("codewright")
      ...
      const item = catalog.provider.get(ProviderV2.ID.codewright)
      if (!item) return
      const hasKey = Boolean(process.env.CODEWRIGHT_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("codewright")),
