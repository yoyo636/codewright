...
import { createCodewright, type ToolPart } from "@codewright-ai/sdk"
...
console.log("🚀 Starting codewright server...")
const codewright = await createCodewright({
  port: 0,
})
console.log("✅ Codewright server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await codewright.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new codewright session...")
    const { client, server } = codewright
    ...
    console.log("✅ Created codewright session:", createResult.data.id)
  ...
  console.log("📝 Sending to codewright:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Codewright response:", JSON.stringify(result, null, 2))
