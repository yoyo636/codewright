...
This is the living design and status document for `@codewright-ai/codemode` and its existing V2 Codewright adapter.
...
### V2 Codewright adapter

This section describes the `v2` branch integration. On `dev`, CodeMode is integrated through
`packages/codewright/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
